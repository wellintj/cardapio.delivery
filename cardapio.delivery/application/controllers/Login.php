...
class Login extends MY_Controller
{
                ...
                foreach ($query['result'] as $row) :
                    ...
                    if ($row->is_active == 1) :
                        ...
                        if ($row->user_role == 1) :
                            ...
                            //add order_types
                            $this->version_changes_m->add_order_types($row->id, $row->account_type);
    ...
    public function user_registration($slug = '')
    {
        ...
        $this->form_validation->set_rules('email', lang('email'), 'trim|required|xss_clean|valid_email|is_unique[users.email]', array('is_unique' => lang('email_already_exists')));
		...
		<strong><i class="fas fa-frown"></i> ' . lang("sorry") . ' </strong> ' . validation_errors() . '
            ...
            $check_limit = $this->security_m->check_limit('registration');
            ...
            $default = $this->admin_m->get_default_package();
            if (!empty($default)) :
                $account_type = $default['id'];
                $type = $default['package_type'];
                $package = $default;
			...
			<strong><i class="fas fa-frown"></i> ' . lang("sorry") . ' ! </strong>  <b>' . str_slug($this->input->post('username', true)) . '</b>' . lang("username_already_exists") . '
                ...
                if ($insert) :
                    ...
                    if ((isset($trial_active['is_payment']) &&  $trial_active['is_payment'] == 1)) :
                        $sub_payment = 1;
                    else :
                        $sub_payment = 0;
                    endif;
                    ...
                    //check referal code
                    $ref_code = isset($_GET['ref']) && !empty($_GET['ref']) ? $_GET['ref'] : '';

                    $this->default_m->add_payment(['user_id' => $insert], $package_info, $payment_type = 0, $ref_code, $sub_payment);
    ...
    protected function new_user_mail($u_info, $package)
    {
        if (isset($this->settings['is_dynamic_mail']) && $this->settings['is_dynamic_mail'] == 1) :
            ...
            $send = $this->email_m->account_create_invoice($this->new_user_mail($u_info, $package)); //send invoice mial
        endif;
...
----------------------------------------------*/
    ...
    public function qr_code($username)
    {
        ...
        if (empty($check['qr_link']) || !file_exists($check['qr_link'])) :
            ...
            $qr_image = $username . '_' . random_string('numeric', 10) . '.png';
    ...
    public function site_qr($is_force = false)
    {

        $check = settings();
        if (empty($check['site_qr_link']) || !file_exists($check['site_qr_link']) || $is_force == true) :
            $this->load->library('ciqrcode');
