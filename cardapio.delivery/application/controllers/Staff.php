...
class Staff extends MY_Controller
{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($type == 'customer') :
				if (preg_match('/^[0-9]+$/', $phone)) {
					$dial_code = isset($_POST['dial_code']) ? $_POST['dial_code'] : "";
					$phone_with_code = $dial_code . $this->input->post('phone', TRUE);
					$phone_without_code = $this->input->post('phone', TRUE);
					$is_staff = $this->admin_m->check_customer_login_status($phone_with_code, $phone_without_code, $password);
				} else {
					$is_staff = $this->admin_m->check_customer_email_login($email, $password);
				}
				$isAdmin = FALSE;
			elseif ($type == 'staff' && filter_var($phone, FILTER_VALIDATE_EMAIL)) :
			...
			elseif ($type == 'delivery') :
				if (!empty($phone) && preg_match('/^[0-9]+$/', $phone)) {
					...
					$phone_with_code = $dial_code . $this->input->post('phone', TRUE);
					$phone_without_code = $this->input->post('phone', TRUE);
					$is_staff = $this->admin_m->delivery_login_with_phone($phone_with_code, $phone_without_code, $password);
				} else {
					$is_staff = $this->admin_m->delivery_login_with_email($email, $password);
				}
				$isAdmin = FALSE;
			else :
				$isAdmin = FALSE;
	...
	public function order_list()
	{
		...
		$total = $this->admin_m->get_customer_order_list(auth('customer_id'), 0, 0, $is_total = 1);
		$config['base_url'] = base_url('staff/order_list');
		$config['total_rows'] = $total;
		$config['per_page'] =  $per_page;
		$this->pagination->initialize($config);

		$data['info'] = $this->admin_m->single_select_by_id(auth('customer_id'), 'customer_list');
		$data['order_list'] = $this->admin_m->get_customer_order_list(auth('customer_id'), $per_page, $offset, 0);
	...
	public function delivery()
	{
		...
		$data['shop_id'] = $this->admin_m->get_shop_id($data['info']['user_id'] ?? 0);
		$data['countries'] = $this->admin_m->select('country');
	...
	public function update_delivery_profile()
	{
		...
		$this->form_validation->set_rules('name', 'Name', 'trim|required|xss_clean');
		if (isset(st()->is_city) && st()->is_city == 1):
	...
	public function get_delivery_order_notification()
	{
		if (empty(auth('is_delivery'))) {
			redirect(base_url('staff-login/delivery'));
		}
		...
		$data['info'] = $this->admin_m->single_select_by_id(auth('staff_id'), 'staff_list');
		$data['shop_id'] = $this->admin_m->get_shop_id($data['info']['user_id'] ?? 0);
		$order = $this->admin_m->get_all_delivery_order_list($data['info']['user_id'], $status = 0, $is_picked = 0);
		if (sizeof($order) > 0) {
	...
	public function get_order_list($type = "new")
	{
		...
		if ($type == "new") {
			...
			$is_picked = 0;
			$data['order_list'] = $this->admin_m->get_all_delivery_order_list($data['info']['user_id'], $status, $is_picked);
			if (!empty($data['order_list']) && sizeof($data['order_list']) > 0) {
				$is_notify = 1;
			}
		} elseif ($type == "accepted") {
			$data['page_title'] = "Accepted OrderList";
		...
		echo json_encode(['st' => 1, 'load_data' => $load_data, 'is_notify' => $is_notify]);
	}
	...
	public function accept_order($uid = null, $dboy_id = null)
	{
		...
		$check = $this->common_m->check_order_status($uid, 'is_db_accept');
		$data = array(
			'dboy_id' => $dboy_id,
		...
		if ((isset($check) && $check->dboy_id == 0) || (isset($check) && $check->dboy_id == auth('staff_id'))) :
			$this->admin_m->update_by_uid($data, $uid);
			__staffAction($uid, $dboy_id, 'accept');
			$this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
		else :
			$this->session->set_flashdata('error', lang('this_job_already_handle_by_someone'));
		endif;
		redirect($_SERVER['HTTP_REFERER']);
	}
	...
	public function picked_order($uid = null, $dboy_id = null)
	{
		...
		$check = $this->common_m->check_order_status($uid);
		...
		if ((isset($check) && $check->dboy_id == 0) || (isset($check->dboy_id) && $check->dboy_id == auth('staff_id'))) :
			$update = $this->admin_m->update_by_uid_dboy_id($data, $uid, $dboy_id);
			__staffAction($uid, $dboy_id, 'picked');
	...
	public function completed_order($uid = null, $dboy_id = null)
	{
		...
		if ((isset($check) && $check->dboy_id == 0) || (isset($check) && $check->dboy_id == auth('staff_id'))) :
			...
			__staffAction($uid, $dboy_id, 'complete');
			$this->session->set_flashdata('success', lang('success_text'));
		else :
			$this->session->set_flashdata('error', lang('this_job_already_handle_by_someone'));
		endif;
	...
	public function cancel_order($uid = null, $dboy_id = null)
	{
		$data = [];
		if (empty($dboy_id) || empty($uid) || empty(auth('is_delivery'))) {
			redirect(base_url('staff-login/delivery'));
		...
		if ((isset($check) && $check->dboy_id == 0) || (isset($check) && $check->dboy_id == auth('staff_id'))) :
			$this->admin_m->update_by_uid_dboy_id($data, $uid, $dboy_id);
			__staffAction($uid, $dboy_id, 'reject');
			$this->session->set_flashdata('success', lang('success_text'));
		else :
			$this->session->set_flashdata('error', lang('this_job_already_handle_by_someone'));
		endif;
