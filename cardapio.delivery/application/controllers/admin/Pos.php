...
class Pos extends MY_Controller
{
	...
	public function index()
	{
		...
		if (empty($_GET['type'])) {
			$config['base_url'] = base_url('admin/pos/ajax_pagination/?type=item');
		} else {
		...
		if (isset($_GET['type']) && ($_GET['type'] == 'packages' || $_GET['type'] == 'package')) {
			$data['is_item'] = 0;
			$data['all_items'] = $this->pos_m->get_my_all_packages(limit($id, 1), $per_page, 0, 0);
		} else {
	...
	public function ajax_pagination()
	{
		...
		if (isset($_GET['type']) && ($_GET['type'] == 'item' || $_GET['type'] == 'items')) {
			$total = $this->pos_m->get_my_all_items($catId = 0, limit($id, 1), 0, 0, $is_total = 1);
		} else {
			$total = $this->pos_m->get_my_all_packages(limit($id, 1), 0, 0, 1);
		}
		$config['base_url'] = base_url('admin/pos/ajax_pagination/');
		$config['total_rows'] = $total;
		$config['per_page'] =  $per_page;
		...
		if (isset($_GET['type']) && ($_GET['type'] == 'item' || $_GET['type'] == 'items')) {
			...
			$data['all_items'] = $this->pos_m->get_my_all_packages(limit($id, 1), $per_page, $offset, 0);
			$data['is_item'] = 0;
		}
	...
	public function add_to_cart($id = 0, $type = 'item')
	{
		$data = array();
		$this->cart_m->__cart($id, $type, $is_pos = 1);
	...
	public function add_to_cart_form()
	{
		...
		$this->cart_m->__cart($id = 0, $type = 'item', $is_pos = 1);
		$item = $this->load->view('backend/pos/inc/ajax_cart_item', $data, true);
		echo json_encode(['st' => 1, 'result' => $item]);
	}
	...
	public function set_service()
	{
		...
		$this->form_validation->set_rules('service_charge', __('service_charge'), 'trim|required|xss_clean');
		if ($this->form_validation->run() == FALSE) {
			echo json_encode(['st' => 0, 'msg' => validation_errors()]);
		} else {
	...
	public function item_details($id, $type = 'item')
	{
		...
		if ($type == 'item') :
			...
			if (!empty($data['item']->shop_id)) :
				...
				$data['url'] =  base_url('admin/pos/add_to_cart_form');
	...
	public function add_order()
	{
		...
		if (isset($_POST['is_draft']) && $_POST['is_draft'] == 1) {
			$is_draft = 1;
			$is_ring = 0;
		} else {
			$is_draft = 0;
		...
		if (!isset($_POST['order_type'])) {
			...
			$data = [
				...
				'discount' => $p->discount,
				'is_item_tax' => $order_info->is_item_tax ?? 0,
			];
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$uid = orderId($get_shop_id ?? 0);

			$order_id = __orderId($uid);

			if (auth('is_staff') == TRUE && !empty(staff_info()->id)) {
				$staff_id = staff_info()->id;
				if (isset($_POST['is_completed'])) {
					$staff_action = 'order_create_complete';
				} else {
					$staff_action = 'create_order';
				}
				$staff_action = 'create_order';
				$action_by = 'staff';
			} else {
			...
			if (isset($order_type['slug']) && $order_type['slug'] == 'room-service') {
				$hotel_id = temp('room')->hotel_id;
				$room_number = temp('room')->room_number;
			} else {
				$hotel_id = 0;
				$room_number = 0;
			}
			...
			$is_paid = isset($payment_type) && $payment_type == 'none' ? 0 : 1;

			$is_payment = isset($_POST['is_completed']) || $is_paid == 1 ? 1 : 0;
			...
			$data = array(
				...
				'is_item_tax' => $shop_info['is_tax'] ?? 0,
				'is_confirm' => isset($_POST['is_completed']) ? 1 : 0,
			...
			if ($insert) {
				...
				if (isset($data['is_draft']) && $data['is_draft'] == 1) :
					echo json_encode(['st' => 1, 'url' => base_url("admin/pos")]);
				else :
	...
	protected function clear_cart()
	{
		...
		$this->session->unset_userdata('temp_data');
		$this->session->unset_userdata('cart');
		$this->session->unset_userdata('is_order_edit');
		return true;
	}
	...
	public function get_item_name()
	{
		...
		if (isset($_GET['type']) && ($_GET['type'] == 'packages' || $_GET['type'] == 'specialities')) {
			$data['is_item'] = 0;
			$total = $this->pos_m->get_my_all_packages(limit($id, 1), 0, 0, 1);
		} else {
			$total = $this->pos_m->get_my_all_items($catId = 0, limit($id, 1), 0, 0, $is_total = 1);
	...
	public function pos_settings()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$pos = [
				...
				'welcome_message' => $this->input->post('welcome_message', true),
				'is_comments' => $this->input->post('is_comments', true) ?? 0,
			];
