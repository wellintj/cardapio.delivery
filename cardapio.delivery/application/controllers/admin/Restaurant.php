...
class Restaurant extends MY_Controller
{
	...
	public function create_restaurant()
	{
		...
		$this->form_validation->set_rules('tax_number', lang('tax_number'), 'trim|xss_clean');
	...
	public function add_restaurant_config()
	{
		...
		if (restaurant()->phone == $this->input->post('phone')) {
			...
			$this->form_validation->set_rules('phone', lang('phone'), 'trim|xss_clean|required|is_unique[restaurant_list.phone]', array('is_unique' => lang('phone_already_exits')));
		}
		...
		if (st()->is_city_delivery):
			$this->form_validation->set_rules('city_id', lang("city_name"), 'trim|required|xss_clean');
		endif;
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect($_SERVER['HTTP_REFERER']);
	...
	public function available_days($shop_id)
	{
		...
		$data['main_content'] = $this->load->view('backend/restaurant/available_days', $data, TRUE);
		$this->load->view('backend/index', $data);
	}

	public function reservation_type($shop_id)
	{

		$data = array();
		$data['page_title'] = "Reservation Types";
		$data['page'] = "Profile";
		$data['data'] = false;
		$data['shop_id'] = $shop_id;
		$data['restaurant'] = $this->admin_m->get_shop_profile_md5($shop_id);
		...
		$data['main_content'] = $this->load->view('backend/restaurant/reservation_type', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	public function add_reservation($shop_id)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect(base_url('admin/restaurant/available_days/' . $shop_id));
		} else {
			...
			if (!empty($days)) :
				...
				foreach ($days as $key => $day) :
					$data = array(
						'days' => $day,
						'start_time' => $_POST['start_time_' . $day][0] ?? '10:00',
						'end_time' => $_POST['end_time_' . $day][0] ?? '10:00',
						'time_config' => !empty($this->get_time_slots($day)) ? json_encode($this->get_time_slots($day)) : '',
	...
	protected function get_time_slots($days)
	{
		$combainedDate = [];
		if (!empty($_POST['start_time_' . $days]) && !empty($_POST['end_time_' . $days])) :
			foreach ($_POST['start_time_' . $days] as $key => $day) :
				if ($key != 0) :
					$combainedDate[] = ['start_time' => $day, 'end_time' => $_POST['end_time_' . $days][$key]];
				endif;
			endforeach;
		endif;
	...
	public function add_order_configuration()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$is_service_charge = $this->input->post('is_service_charge');
			$ids = $this->input->post('ids');
			$i = 1;
			foreach ($ids as $key => $payment) :
				$data = [
					'is_payment' => isset($is_payment[$payment]) ? 1 : 0,
					'is_required' => isset($is_required[$payment]) ? 1 : 0,
					'is_service_charge' => isset($is_service_charge[$payment]) ? 1 : 0,
				];
	...
	public function add_reservation_type($shop_id)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect(base_url('admin/restaurant/reservation_type/' . $shop_id));
		} else {
	...
	public function edit_reservation($uid)
	{
		$data = array();
		$data['page_title'] = "Reservation List";
		$data['page'] = "Reservation List";
		$data['page_type'] = "Today";
		$data['data'] = $this->admin_m->single_select_by_uid($uid, 'order_user_list');
		$data['reservation_types'] = $this->common_m->get_all_by_shop_id(restaurant()->id, 'reservation_types');
		$data['main_content'] = $this->load->view('backend/restaurant/edit_reservation', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	public function add_edit_reservation()
	{
		...
		if ($this->form_validation->run() == false) {
			$this->session->set_flashdata('error', validation_errors());
			redirect(base_url('admin/restaurant/add_edit_reservation/'));
		} else {
			...
			if (isset($_POST['uid']) && !empty($_POST['uid'])) {
				...
				$uid = $shop_id . random_string('numeric', 7);
			}
			...
			$data = [
				...
				'comments' => $comments,
				'order_type' => Order::type('reservation'),
				'shop_id' => restaurant()->id,
				'is_ring' => 1,
			];
			...
			if (isset($id) && $id != 0) {
				...
				$new_arr = ['created_at' => d_time()];
				$insert = $this->common_m->insert(array_merge($data, $new_arr), 'order_user_list');
			}

			if ($insert) {
				$this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
				redirect(base_url('admin/restaurant/reservation_list/'));
			} else {
				$this->session->set_flashdata('error', !empty(lang('error_text')) ? lang('error_text') : 'Somethings Were Wrong!!');
				redirect(base_url('admin/restaurant/add_edit_reservation/'));
			}
	...
	public function reservation_list()
	{
		...
		$total = $this->admin_m->get_my_reservation_list_by_id(restaurant()->id, 0, 0, $is_total = 1);
		$config['base_url'] = base_url('admin/restaurant/reservation_list');
		$config['total_rows'] = $total;
		$config['per_page'] =  $per_page;
		$this->pagination->initialize($config);


		$data['order_list'] = $this->admin_m->get_my_reservation_list_by_id(restaurant()->id, $per_page, $offset, 0);
		$data['main_content'] = $this->load->view('backend/restaurant/reservation_list', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
...
----------------------------------------------*/
	...
	public function order_details_ajax($id)
	{
		...
		if (!empty($id)) {
			...
			$data['order_details'] = $this->admin_m->single_select_by_uid($id, 'order_user_list');
			$data['shop_id'] = $data['order_details']['shop_id'];
			$data['order_id'] = $id;
			$data['item_list'] = $this->admin_m->get_item_list_by_order_id($id, $data['shop_id']);

			$data['all_items'] = $this->admin_m->get_all_items_for_order($per_page, 0, 0);
			$data['settings'] = $this->admin_m->get_user_settings();

			$load_data = $this->load->view('backend/order/inc/order_details_thumb', $data, TRUE);

			echo json_encode(['st' => 1, 'result' => $load_data]);
		}
	}
	...
	public function order_status($id, $status)
	{
		...
		if ($status == 1) {
			...
			if (restaurant()->es_time == 0) :
				...
				$time_data = array(
					'es_time' => $time ?? $order_info['es_time'],
					'time_slot' => $time_slot ?? $order_info['time_slot'],
					'is_preparing' => 0,
			...
			$date = 'accept_time';
			$this->admin_m->get_order_details($order_info['id'], $shop_id, $st = 0, $encrypt = false);
		} elseif ($status == 2) {
			$date = 'completed_time';

			$this->admin_m->get_order_details($order_info['id'], $shop_id, $st = 1, $encrypt = false);
		} elseif ($status == 3) {
			$reject_id = $_POST['reason_id'] ?? '';
			$reject_msg = $_POST['reason_msg'] ?? '';
			$reject_reason = ['reason_ids' => $reject_id, 'reject_msg' => $reject_msg];
			$date = 'cancel_time';
			$this->admin_m->get_order_details($order_info['id'], $shop_id, $st = 2, $encrypt = false);
		}
		...
		$data = array(
			...
			'action_by' => $action_by,
			'reject_reason' => isset($reject_reason) ? json_encode($reject_reason) : '',
			'is_draft' => 0,
		...
		try {
			$this->system_model->whatsapp_message($status, $order_info); // WhatsApp msg for order status
		} catch (Exception $e) {
			log_message('error', 'WhatsApp message error: ' . $e->getMessage());
		}

		try {
			$this->version_changes_m->pusher(restaurant()->id, 'order_status'); // Pusher for order status
		} catch (Exception $e) {
			log_message('error', 'Pusher error: ' . $e->getMessage());
		}

		try {
			$this->user_email_m->send_order_mail($order_info['uid'], $status); // Send email for order status
		} catch (Exception $e) {
			log_message('error', 'Email sending error: ' . $e->getMessage());
		}

		try {
			$sms = $this->system_model->sms($order_info['uid'], $status); // Send SMS
		} catch (Exception $e) {
			log_message('error', 'SMS sending error: ' . $e->getMessage());
		}


		__staffAction($id, $staff_id, $staff_action);
		if ($change) {
			redirect($_SERVER['HTTP_REFERER']);
		}
	}
	...
	protected function order_completed($id, $order_info)
	{
		...
		if (isset($order_info['accept_time']) && $order_info['accept_time'] == '0000-00-00 00:00:00' || empty($order_info['accept_time'])) {
			$arr = array(
				'status' => 1,
				'accept_time' => d_time(),
			);
	...
	public function reservation_order_status($id, $status)
	{
		...
		if ($change) {
			@$this->user_email_m->send_reservation_mail($id, $status);
	...
	public function order_status_by_ajax($id, $status)
	{
		...
		if ($change) {
			...
			$this->user_email_m->send_order_mail($order_info['uid'], $status);

			$this->system_model->whatsapp_message($status, $order_info);
			$this->version_changes_m->pusher($shop_id, 'order_status', $order_id ?? 0);

			__staffAction($order_info['uid'], $staff_id, $staff_action);
	...
	public function profile()
	{
		...
		$data['cityList'] = $this->admin_m->select('restaurant_city_list');
	...
	public function delivery_config()
	{
		$data = [
			...
			'delivery_area_style' => isset($_POST['delivery_area_style']) ? $_POST['delivery_area_style'] : 0,
		];
	...
	public function add_configuration()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$is_email_based = $this->input->post('is_email_based', TRUE);
			$is_quick_access = $this->input->post('is_quick_access', TRUE);
			$is_allergen_image = $this->input->post('is_allergen_image', TRUE);
			$is_order_qr = $this->input->post('is_order_qr', TRUE);
			...
			$others_config = [
				'is_allergen_image' => isset($is_allergen_image) ? 1 : 0,
				'is_kds_pending_orders' => isset($_POST['is_kds_pending_orders']) ? 1 : 0,
			];
			$data = array(
				...
				'is_customer_login' => isset($is_customer_login) ? $is_customer_login : 0,
				...
				'is_login' => isset($is_login) ? 1 : 0,
				'is_email_based' => isset($is_email_based) ? 1 : 0,
				'is_quick_access' => isset($is_quick_access) ? 1 : 0,
				'guest_config' => json_encode($guest_config),
				'others_config' => json_encode($others_config),
				'is_order_qr' => isset($is_order_qr) ? 1 : 0,
			);
			$insert = $this->admin_m->update($data, $id, 'restaurant_list');
			if ($insert) {
				$this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
	...
	public function add_payment_method()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$moyasar = array(
				'moyasar_public_key' => $this->input->post('moyasar_public_key', TRUE),
				'moyasar_secret_key' => $this->input->post('moyasar_secret_key', TRUE),
			);


			$midtrans = array(
				'client_key' => $this->input->post('client_key', TRUE),
				'server_key' => $this->input->post('server_key', TRUE),
				'is_midtrans_live' => $this->input->post('is_midtrans_live', TRUE) != 1 ? 0 : 1,
			);

			$cashfree = array(
				'cashfree_secret_key' => $this->input->post('cashfree_secret_key', TRUE),
				'cashfree_app_id' => $this->input->post('cashfree_app_id', TRUE),
				'is_cashfree_live' => $this->input->post('is_cashfree_live', TRUE) != 1 ? 0 : 1,
			);


			$offline = array(
				'offline_details' => $this->input->post('offline_details'),
				'is_transaction_field' => $this->input->post('is_transaction_field', TRUE) ? 1 : 0,
				'is_document_field' => $this->input->post('is_document_field', TRUE) ? 1 : 0,
				// Adicionando campos para o PIX
				'pix_key' => $this->input->post('pix_key', TRUE),
				'city' => $this->input->post('city', TRUE),
				'pix_description' => $this->input->post('pix_description', TRUE),
			);

			$myfatoorah = array(
				'myfatoorah_api_key' => $this->input->post('myfatoorah_api_key', TRUE),
				'vccode' => $this->input->post('vccode', TRUE) ?? "KWT",
				'is_myfatoorah_live' => $this->input->post('is_myfatoorah_live', TRUE) != 1 ? 0 : 1,
			);

			$pix = array(
				'pix_key' => $this->input->post('pix_key', TRUE),
				'city' => $this->input->post('pix_city', TRUE),
				'pix_description' => $this->input->post('pix_description', TRUE),
			);
			...
			$data = array(
				...
				'is_moyasar' => $this->input->post('is_moyasar', TRUE) != 1 ? 0 : 1,
				'is_offline' => $this->input->post('is_offline', TRUE) != 1 ? 0 : 1,

				'is_midtrans' => $this->input->post('is_midtrans', TRUE) != 1 ? 0 : 1,
				...
				'moyasar_config' => json_encode($moyasar),
				'offline_config' => json_encode($offline),

				'midtrans_config' => json_encode($midtrans),
				'cashfree_config' => json_encode($cashfree),
				'myfatoorah_config' => json_encode($myfatoorah),
				'iyzico_config' => json_encode($iyzico),
				'pix_config' => json_encode($pix),
			);
			...
			if ($insert) {
				...
				if($is_pix_estatico == 1 && !empty($this->input->post('pix_key', TRUE))) {
					...
					if($pix_exists) {
						// Atualizar
						$this->db->where('restaurant_id', $id)->update('pix_config', $pix_data);
					} else {
						// Inserir
						$this->db->insert('pix_config', $pix_data);
					}
				}
	...
	public function add_dboy()
	{
		...
		if (st()->is_city_delivery == 1):
			$this->form_validation->set_rules('city_id', lang('city'), 'required|trim|xss_clean');
		endif;
		if ($this->form_validation->run() == FALSE) {
			...
			if ($dphone != 0 && $dphone != $phone) {
				if ($check_staff_phone == 1) :
					__request(lang('phone_already_exits'), 0, '');
					exit();
				endif;
			}
			...
			if ($insert) {
				__request(lang('success_text'), 1, base_url("admin/restaurant/dboy_list"));
			} else {
				__request(lang('error_text'), 0, base_url("admin/restaurant/dboy_list"));
			}
		}
	}
	...
	public function add_customer()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($insert) {
				__request(lang('success_text'), 1, base_url('admin/restaurant/customer_list'));
			} else {
				__request(lang('error_text'), 0, base_url('admin/restaurant/customer_list'));
			}
		}
	}
	...
	public function add_order_item($uid, $id)
	{
		...
		$shop_info = single_select_by_id(restaurant()->id, 'restaurant_list');
		...
		// Calculate the new item's subtotal
		$new_sub_price = $price * $qty;
		$old_subtotal = $order_info['sub_total'] + $order_info['tax_fee'];


		// Tax calculation logic based on shop settings
		if ($shop_info['is_tax'] == 1) {

			$tax_percent = $item_info['tax_fee'] ?? 0;
			$tax_status = $item_info['tax_status'];
			$item_price = $new_sub_price;
			$tax_fee = __taxCalc($item_price, $tax_percent, $tax_status);
			$total_subtotal = $tax_status == '+' ? $item_price - $tax_fee : $item_price;
			$is_item_tax = 1;

			$grandTax = $order_info['tax_fee'] + $tax_fee;
			$grandsubTotal = $order_info['sub_total'] + $total_subtotal;
		} else {
			// Calculate tax for the entire subtotal
			$old_subtotal = $order_info['sub_total'] + $order_info['tax_fee'];
			$new_subtotal = $new_sub_price;
			$grandSubtotal = $old_subtotal + $new_subtotal;
			$tax_percent = $shop_info['tax_fee'] ?? 0;
			$tax_status = $shop_info['tax_status'] ?? '+';
			$tax_fee = __taxCalc($grandSubtotal, $tax_percent, $tax_status);
			$is_item_tax = 0;
			...
			if (tax_type() == 'seperate') {
				...
				$grandsubTotal = $tax_status == '+' ? $grandSubtotal - $tax_fee : $grandSubtotal;
			}
		}

		// Calculate the total using grand_total
		$total_price = grand_total(
			$grandsubTotal,
			$order_info['delivery_charge'],
			$order_info['discount'],
			$grandTax,
			$order_info['coupon_percent'],
			$order_info['tips'],
			$order_info['order_type'],
			$tax_status,
			$order_info['is_pos'],
			$is_item_tax,
			$order_info['service_charge']
		);
		...
		// Update the user order with the new totals
		$user_order = array(
			'sub_total' => number_format($grandsubTotal, 2), // Updated subtotal
			'total' => number_format($total_price, 2), // Grand total
			'tax_fee' => number_format($grandTax, 2), // Updated tax fee
		);
		...
		$check_settings = shop($data['shop_id'])->stock_status;

		// if (isset($check_settings) && $check_settings == 1) :
		// 	if (isset($data['is_package']) && $data['is_package'] == 1) :
		// 		$info = single_select_by_id($id, 'item_packages');
		// 		$up_data = ['remaining' => $info['remaining'] + $data['qty']];
	...
	{
		// Delete the item from the order_item_list

		$deleted_item = $this->admin_m->single_select_by_id($id, 'order_item_list');
		if (!$deleted_item) {
			$this->session->set_flashdata('error', 'Item not found!');
			redirect($_SERVER['HTTP_REFERER']);
		}

		$item_info = $this->admin_m->single_select_by_id($deleted_item['item_id'], 'items');
		$order_info = $this->admin_m->single_select_by_uid($uid, 'order_user_list');

		// Calculate the item's subtotal
		$item_subtotal = $deleted_item['item_price'] * $deleted_item['qty'];

		// Determine tax calculation based on shop settings
		$shop_info = single_select_by_id(restaurant()->id, 'restaurant_list');

		$new_sub_price = $item_subtotal;
		$old_subtotal = $order_info['sub_total'] + $order_info['tax_fee'];


		// Tax calculation logic based on shop settings
		if ($shop_info['is_tax'] == 1) {
			$tax_percent = $item_info['tax_fee'] != 0 ? $item_info['tax_fee'] : $shop_info['tax_fee'];
			$tax_status = $item_info['tax_status'] ?? '+';
			$item_price = $deleted_item['sub_total'] * $deleted_item['qty'] ?? 0;

			$tax_fee = __taxCalc($item_price, $tax_percent, $tax_status);
			$total_subtotal = $tax_status == '+' ? $item_price - $tax_fee : $item_price;
			$is_item_tax = 1;

			$grandTax =  abs($order_info['tax_fee'] - $tax_fee);
			$grandsubTotal = $order_info['sub_total'] - $total_subtotal;
		} else {
			$new_subtotal = $new_sub_price;
			$total_subtotal = $old_subtotal - $new_subtotal;
			$tax_percent = $shop_info['tax_fee'] ?? 0;
			$tax_status = $shop_info['tax_status'] ?? '+';
			$tax_fee = __taxCalc($total_subtotal, $tax_percent, $tax_status);
			$is_item_tax = 0;
			...
			if (tax_type() == 'seperate') {
				$grandsubTotal = $total_subtotal;
			} else {
				$grandsubTotal = $tax_status == '+' ? $total_subtotal - $tax_fee : $total_subtotal;
			}
		}
		...
		$update = $this->admin_m->update($user_order, $order_info['id'], 'order_user_list');
		if ($update) {
		...
		if (isset($delete)) :
			...
			$this->session->set_flashdata('error', !empty(lang('error_text')) ? lang('error_text') : 'Something Went Wrong!!');
			redirect($_SERVER['HTTP_REFERER']);
		endif;
	}
	...
	public function edit_order($order_id)
	{
		...
		foreach ($item_details as $key => $item) {
			if (isset($item['is_extras']) && $item['is_extras'] == 1) {
				...
				$ids = $item['id'] . '-' . $item['item_price'];
				$extra_info = @$this->admin_m->get_extras_name_by_id($extra_id, $item['id']);
			...
			$tax_status = restaurant()->tax_status ?? '+';


			if ($item['is_package'] == 0) {
				$item_info =  $this->admin_m->single_select_by_id($item['item_id'], 'items');
			} else {
				$item_info =  $this->admin_m->single_select_by_id($item['package_id'], 'item_packages');
			}
			...
			$cart_data = array(
				...
				'extra_price' => $price,
				'is_extras' => $item['is_extras'] ?? 0,
				'extra_id' => $item['extra_id'] ?? '',
				'extra_qty' => $item['extra_qty'] ?? '',

				'sizes' => ['size_slug' => $item_size ?? ''],
				'options' => array('veg' => $item['veg_type']),
				'is_variants' => !empty($item['is_size']) ? $item['is_size'] : 0,
				'sizes' => ['size_slug' => $item['size_slug'], 'is_size' => !empty($item['is_size']) ? $item['is_size'] : 0, 'size_price' => $item['item_price']],



				'tax_fee' => !empty($item_info['tax_fee']) ? $item_info['tax_fee'] : 0,
				'sub_total' => !empty($item_info['sub_total']) ? $item_info['sub_total'] : 0,
				'tax_status' => !empty($item_info['tax_status']) ? $item_info['tax_status'] : '+',
				'is_item_tax' => !empty($item['is_item_tax']) ? $item['is_item_tax'] : 0,
	...
	public function delete_single_item($id, $table)
	{
		...
		if ($del) {
			$this->session->set_flashdata('success', __('success_text'));
			redirect($_SERVER['HTTP_REFERER']);
		} else {
			$this->session->set_flashdata('error', __('error_text'));
			redirect($_SERVER['HTTP_REFERER']);
		}
	}
	...
	public function reviews()
	{
		...
		$data['data'] = FALSE;
		$data['review_list'] = $this->admin_m->get_my_shop_reviews($_ENV['ID']);
		$data['total_rating'] = $this->common_m->total_shop_rating($_ENV['ID']);
		$data['total_review'] = $this->common_m->total_shop_rating($_ENV['ID'], 'total');
		$data['settings'] = settings();
		$data['main_content'] = $this->load->view('backend/restaurant/shop_reviews', $data, TRUE);
		$this->load->view('backend/index', $data);
	}

	public function staff_activities($uid)
	{
		$data = array();
		$data['page_title'] = "Staff";
		$data['page'] = "Staff";
		$data['data'] = FALSE;
		$data['staff_info'] = $this->admin_m->single_select_by_uid_row($uid, 'staff_list');
		$data['staff_list'] = $this->admin_m->get_staff_activities($uid);
		$data['main_content'] = $this->load->view('backend/restaurant/staff_activities', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	{
		$data = array();
		$data['page_title'] = "Vendor Questions";
		$data['page'] = "Profile";
		$data['data'] = FALSE;
		$data['question_list'] = $this->admin_m->select_all_by_vendor_ln($_ENV['ID'], 'vendor_question_list');
		$data['main_content'] = $this->load->view('backend/restaurant/vendor_question_list', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	public function edit_question($id)
	{
		...
		$data['page'] = "Profile";
		$data['data'] = $this->admin_m->single_select_by_id($id, 'vendor_question_list');
		valid_user($data['data']['user_id']);
		$data['question_list'] = $this->admin_m->select_all_by_vendor_ln($_ENV['ID'], 'vendor_question_list');
		$data['main_content'] = $this->load->view('backend/restaurant/vendor_question_list', $data, TRUE);
		$this->load->view('backend/index', $data);
	...
	public function send_mail($type = 'reservation')
	{
		is_test();
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$id = $this->input->post('id');
			$is_mail = isset($_POST['is_mail']) ? 1 : 0;
			...
			if ($type == 'reservation' && $is_mail == 1 && !empty($uid)) :
				$this->user_email_m->send_reservation_mail($uid, 5);
			endif;

			$send = $this->user_email_m->custom_mail($id, $data);
			if ($send) {
				__request('', $status = 1);
			} else {
				__request('', $status = 0);
	...
	public function add_questions()
	{
		...
		$this->form_validation->set_rules('type', lang('type'), 'trim|xss_clean|required');
		$this->form_validation->set_rules('access_for', lang('access_for'), 'trim|xss_clean|required');
		$this->form_validation->set_rules('language', lang('language'), 'trim|xss_clean|required');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect(base_url('admin/restaurant/questions'));
		} else {
			$data = [
				'title' => $this->input->post('title'),
				'type' => $this->input->post('type'),
				'access_for' => $this->input->post('access_for'),
			...
			if ($id == 0) {
				$insert = $this->admin_m->insert($data, 'vendor_question_list');
			} else {
				$insert = $this->admin_m->update($data, $id, 'vendor_question_list');
			}
	...
	public function menu_list()
	{
		...
		$data['page'] = "Profile";
		$data['data'] = FALSE;
		$data['menu_list'] = $this->admin_m->get_vendor_menu($_ENV['ID']);
		$data['menuList'] = $this->common_m->get_all_by_shop_id($_ENV['ID'], 'vendor_menu_list');
		$data['main_content'] = $this->load->view('backend/restaurant/menu_list', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	public function add_menu()
	{
		...
		$id = $this->input->post('id');
		$this->form_validation->set_rules('title', lang('title'), 'trim|xss_clean|required');
		$this->form_validation->set_rules('url', lang('url'), 'trim|xss_clean');
		if ($this->form_validation->run() == FALSE) {
			__request(validation_errors(), 0, '');
		} else {
			$language = $_POST['language'] ?? 'english';
			$isClone = $_POST['is_clone'] ?? 0;
			$is_extranal = isset($_POST['is_extranal_url']) ? 1 : 0;
			$data = [
				'title' => $this->input->post('title'),
				'url' => $this->input->post('url'),
				'is_dropdown' => $this->input->post('is_dropdown') ?? 0,
				'is_quick_link' => $this->input->post('is_quick_link') ?? 0,
				'dropdown_id' => $this->input->post('dropdown_id') ?? 0,
				'is_extranal_url' => $is_extranal,
				'user_id' => auth('id'),
				'shop_id' => $_ENV['ID'],
				'language' => $language,
				'created_at' => d_time()
			...
			if (isset($isClone) && $isClone == 1) {
				$insert = $this->admin_m->insert($data, 'vendor_menu_list');
			} else {

				if ($id == 0) {
					$insert = $this->admin_m->insert($data, 'vendor_menu_list');
				} else {
					$insert = $this->admin_m->update($data, $id, 'vendor_menu_list');
				}
			}

			if ($insert) {
				__request(lang('success_text'), 1, base_url('admin/restaurant/menu_list'));
			} else {
				__request(lang('error_text'), 0, base_url('admin/restaurant/menu_list'));
			}
		}
	}


	public function change_table_info()
	{
		$id = $this->input->post('id');
		$this->form_validation->set_rules('table_no', lang('table'), 'trim|xss_clean|required');
		$this->form_validation->set_rules('total_person', lang('person'), 'trim|xss_clean|required');
		$this->form_validation->set_rules('id', lang('order_id'), 'trim|xss_clean|required');
		if ($this->form_validation->run() == FALSE) {
			__request(validation_errors(), 0, '');
		} else {
			$data = [
			...
			if ($insert) {
				...
				__request(lang('error_text'), 0, '');
			}
		}
	}

	public function verify_payment($uid, $payment_id)
	{
		is_test();
		$update = $this->admin_m->update_by_uid(['is_payment' => 1, 'use_payment' => 1], $uid);
		$update = $this->admin_m->update(['status' => 'completed'], $payment_id, 'order_payment_info');
		if ($update) {
			$this->session->set_flashdata('success', __('success_text'));
			redirect($_SERVER['HTTP_REFERER']);
		} else {
	...
	{
		$status = $this->input->post('status', true);
		$payment_by = $this->input->post('payment_by', true);
	...
	public function delivery_csv_template()
	{
		$data = array('area', 'cost');
		generate_csv($data, 'delivery_area');
	}


	public function import_delivery_cvs()
	{
		is_test();
		$data = array('area', 'cost');
		$static = ['shop_id' => restaurant()->id, 'user_id' => auth('id')];
		$insert = import_csv($data, 'delivery_area_list', $static);
		if ($insert) {
			$this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
			redirect($_SERVER['HTTP_REFERER']);
		}
	...
	public function available_type()
	{
		$type = $this->input->post('available_type', TRUE);
		$id = $this->input->post('id', TRUE);
		$update = $this->admin_m->update(['available_type' => $type], $id, 'restaurant_list');
	...
	/**
	 * Validar credenciais do Mercado Pago para PIX dinâmico
	 *
	 * @return void
	 */
	public function validate_mercado_credentials()
	{
		...
		// Buscar credenciais do Mercado Pago do restaurante atual
		try {
			$restaurant_data = restaurant();
			if (empty($restaurant_data) || empty($restaurant_data->id)) {
				echo json_encode([
					'valid' => false,
					'error' => 'Erro de sessão. Faça login novamente e tente novamente.'
				]);
				return;
			}

			// get_restaurant_info() espera o ID do usuário, não do restaurante
			$restaurant_info = $this->admin_m->get_restaurant_info($restaurant_data->user_id);
			$mercado_config = [];

			if (!empty($restaurant_info['mercado_config'])) {
				$mercado_config = json_decode($restaurant_info['mercado_config'], true);
			}
		} catch (Exception $e) {
			echo json_encode([
				'valid' => false,
				'error' => 'Erro ao acessar dados do restaurante: ' . $e->getMessage()
			]);
			return;
		}
		...
		if (empty($mercado_config['access_token'])) {
			echo json_encode([
				'valid' => false,
				'error' => 'Credenciais do Mercado Pago não configuradas. Configure primeiro o Mercado Pago na seção acima.'
			]);
			return;
		}
		...
		$validation = $this->mercado_pix_m->validate_mercado_config($mercado_config);
