...
class Auth extends MY_Controller
{
    ...
    public function profile($user_id)
    {
        ...
        if (auth('user_role') == 1 && $user_id != md5(auth('id'))):
            ...
            $data['shop_id'] = $data['restaurant']['id'] ?? 0;
            $data['earning_list'] = $this->admin_m->get_my_earnings($data['shop_id'], $data['year'], $data['month']);
        endif;
    ...
    public function subscriptions()
    {
        ...
        $data['order_types'] = $this->admin_m->get_order_types();
    ...
    public function item_delete($id, $table)
    {
        ...
        if ($table === 'staff_list') {
            ...
            $this->db->delete('staff_order_activity_logs');
        }
        ...
        if ($del) {
            ...
            $this->session->set_flashdata('success', lang('deleted_successfully'));
            redirect($_SERVER['HTTP_REFERER']);
        } else {
    ...
    public function features_toggle($id, $value, $language)
    {
        is_test();
        if ($value == 0) {
            $data_value = 1;
        ...
        $this->admin_m->update_feature($data, $id, 'users_active_features', $language);
    ...
    public function add_features_info()
    {
        ...
        $getF = $this->admin_m->get_manage_feature_by_language($language);

        if (isset($getF) && $getF == 1):
            foreach ($heading as $key => $value) {
                ...
                $update = $this->admin_m->update_feature($data, $id, 'users_active_features', $language);
            }
        else:
            ...
            $update = $this->admin_m->insert_all($insertData, 'users_active_features');
        endif;
        if ($update) {
            $this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
    ...
    public function email_settings()
    {
        ...
        $data = array();
        $data['page_title'] = "Orders Email Settings";
        $data['page'] = "Settings";
        $data['u_info'] = $this->admin_m->get_user_info();
        $data['settings'] = $this->admin_m->get_user_settings();
        $data['main_content'] = $this->load->view('backend/users/orders_email_settings', $data, true);
        $this->load->view('backend/index', $data);
    }
    ...
    {
        is_test();
        $this->form_validation->set_rules('subject', lang("subject"), 'trim|xss_clean|required');
        $this->form_validation->set_rules('message', lang("order_mail"), 'trim|required');
        $this->form_validation->set_rules('reservation_message', lang('reservation_mail'), 'trim|required');
        $this->form_validation->set_rules('order_receiver_mail', lang("order_receiver_mail"), 'trim|required');
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', validation_errors());
            redirect(base_url('admin/auth/email_settings'));
        } else {
            ...
            $is_welcome_msg = $this->input->post('is_welcome_msg', true);
            $is_customer_mail = $this->input->post('is_customer_mail', true);
            $mail_data = array(
                'subject' => $this->input->post('subject', true),
                'message' => json_encode($this->input->post('message')),
                'welcome_mail' => json_encode($this->input->post('welcome_mail')),
                'order_receiver_mail' => $this->input->post('order_receiver_mail'),
                'otp_mail' => json_encode($this->input->post('otp_mail')),
                ...
                'is_welcome_msg' => isset($is_welcome_msg) ? $is_welcome_msg : 0,
                'reservation_message' => json_encode($this->input->post('reservation_message')),
                'welcome_reservation_message' => json_encode($this->input->post('welcome_reservation_message')),
                'rev_subject' => $this->input->post('rev_subject'),
                'is_rev_customer_mail' => isset($_POST['is_rev_customer_mail']) ? $_POST['is_rev_customer_mail'] : 0,
                'is_rev_owner_mail' => isset($_POST['is_rev_owner_mail']) ? $_POST['is_rev_owner_mail'] : 0,
            );
            ...
            if ($id == 0) {
                $insert = $this->admin_m->__insert($data, 'user_settings');
            } else {
                $insert = $this->admin_m->__update($data, $id, 'user_settings');
            }
    ...
    public function whatsapp_support()
    {
        check_valid_user();
        $data = array();
        $data['page_title'] = "Whatsapp Support";
        $data['page'] = "Whatsapp Config";
        $data['restaurant'] = $this->admin_m->get_my_restaurant(md5(auth('id')));
        $data['main_content'] = $this->load->view('backend/users/whatsapp_support', $data, true);
        $this->load->view('backend/index', $data);
    }
    ...
    public function payment_config()
    {
        ...
        $data['offline_list'] = $this->admin_m->select_all_by_user('restaurant_offline_payemnt_list');
    ...
    public function add_whatsapp_config()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', validation_errors());
            redirect(base_url('admin/auth/whatsapp_share'));
        } else {
            $id = $this->input->post('id', true);
            $is_whatsapp = $this->input->post('is_whatsapp', true);
            $data = array(
                'whatsapp_number' => $this->input->post('whatsapp_number', true),
                'whatsapp_msg' => json_encode($this->input->post('whatsapp_msg')),
    ...
    public function add_whatsapp_support()
    {
        is_test();
        $this->form_validation->set_rules('whatsapp_number', lang('whatsapp_number'), 'trim|required');
        $this->form_validation->set_rules('welcome_message', lang('welcome_message'), 'trim|required');
        $this->form_validation->set_rules('dial_code', lang('dial_code'), 'trim|required');
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', validation_errors());
            redirect(base_url('admin/auth/whatsapp_support'));
        } else {
            $id = $this->input->post('id', true);
            ...
            $data = ['whatsapp_support' => json_encode($arrayD)];
            if ($id == 0) {
                $insert = $this->admin_m->insert($data, 'restaurant_list');
            } else {
                $insert = $this->admin_m->update($data, $id, 'restaurant_list');
    ...
    public function add_whatsapp_message_config()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_config = array(
                ...
                'welcome_msg' => json_encode($this->input->post('welcome_msg')),
                'accept_msg' => json_encode($this->input->post('accept_msg')),
                'complete_msg' => json_encode($this->input->post('complete_msg')),
                'cancled_msg' => json_encode($this->input->post('cancled_msg')),
                'is_welcome' => isset($_POST['is_welcome']) ? 1 : 0,
    ...
    public function add_email_settings()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $smtp = array(
                ...
                'mailtrap_api_key' => $this->input->post('mailtrap_api_key', true),
    ...
    public function add_extra_config()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $jsData = [
                ...
                'system_default_language' => isset($_POST['system_default_language']) ? $_POST['system_default_language'] : 'english',
            ];
            ...
            if ($update) {
                if (isset($_POST['system_default_language']) && !empty($_POST['system_default_language'])) {
                    $this->session->set_userdata('site_lang', $_POST['system_default_language']);
                }
                $this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
                redirect($_SERVER['HTTP_REFERER']);
            } else {
    ...
    public function add_layouts()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $is_theme_toggle = $this->input->post('is_theme_toggle', true);
            ...
            $user = [
                ...
                'is_theme_toggle' => isset($is_theme_toggle) ? $is_theme_toggle : 0,
    ...
    {
        is_test();
        $this->form_validation->set_rules('name', lang('restaurant_name'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('country_id', lang('country'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $iUser = $this->admin_m->update($users, auth('id'), 'users');
            $iResturant = $this->admin_m->update($restaurant, restaurant()->id, 'restaurant_list');

            if ($iUser && $iResturant) {
                $this->add_available_days();
                redirect(base_url('dashboard'));
            }
        }
    }
    ...
    protected function add_available_days()
    {
        $days = get_days();
        $i = 0;
    ...
    public function add_tips_config()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_config = array(
                ...
                'is_service_charge' => isset($_POST['is_service_charge']) ? 1 : 0,
                'service_charge' => $this->input->post('service_charge', true),
                'service_charge_type' => $this->input->post('service_charge_type', true),
            );
    ...
    public function delete_img($id, $table)
    {
        ...
        if (!empty($img)) {
            ...
            echo json_encode(array('st' => 1, 'msg' => lang('deleted_successfully')));
        }
    }
    ...
    public function slider()
    {
        ...
        $data['page'] = "Settings";
        $data['vendor_slider_list'] = $this->admin_m->select_by_shop('vendor_slider_list');
        $data['main_content'] = $this->load->view('backend/users/slider', $data, true);
        $this->load->view('backend/index', $data);
    }
    ...
    {
        is_test();
        $this->form_validation->set_rules('title', __('title'), 'required|trim|xss_clean');
        $this->form_validation->set_rules('status', __('status'), 'required|trim|xss_clean');
        $this->form_validation->set_rules('external_url', __('external_url'), 'trim|xss_clean');
        if ($this->form_validation->run() == false) {
            __request(validation_errors(), 0, '');
        } else {

            $id = $this->input->post('id');
            if (isset($_FILES['file']['name'][0]) && empty($_FILES['file']['name'][0]) && $id == 0):
                __request(__('required_fields'), 0, '');
                exit();
            endif;
            $data = array(
                'title' => $this->input->post('title', true),
            ...
            if ($id == 0) {
                $insert = $this->admin_m->insert($data, 'vendor_slider_list');
            } else {
                $insert = $this->admin_m->update($data, $id, 'vendor_slider_list');
            }

            if ($insert) {
                $this->upload_m->upload_img($insert, 'vendor_slider_list');
                __request(__('success_text'), 1, 'admin/auth/slider');
            } else {
    ...
    {
        is_test();
        $this->form_validation->set_rules('city_id', __('city_name'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER['HTTP_REFERER']);
        } else {
    ...
    public function add_offline_method()
    {
        is_test();
        $this->form_validation->set_rules('name', __('offline_name'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            ...
            $insert = $this->admin_m->insert($data, 'restaurant_offline_payemnt_list');
            if ($insert) {
                $this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
                redirect($_SERVER['HTTP_REFERER']);
            } else {
    ...
    public function checkout_config()
    {
        $data = array();
        $data['page_title'] = "Checkout Config";
        $data['page'] = "Settings";
        $data['main_content'] = $this->load->view('backend/users/checkout_config', $data, true);
        $this->load->view('backend/index', $data);
    }
    ...
    public function promotion()
    {
        ...
        $data['page'] = "Promotion";
        $data['customer_list'] = $this->admin_m->get_mailed_customers();
        $data['main_content'] = $this->load->view('backend/users/promotion', $data, true);
        $this->load->view('backend/index', $data);
    }
    ...
    {
        is_test();
        $this->form_validation->set_rules('mails[]', __('email'), 'required|trim|xss_clean');
        $this->form_validation->set_rules('subject', __('subject'), 'required|trim|xss_clean');
        $this->form_validation->set_rules('message', __('message'), 'required|trim|xss_clean');
        if ($this->form_validation->run() == false) {
            __request(validation_errors(), 0, '');
        } else {
            $mail = $this->input->post('mails', true);
            $subject = $this->input->post('subject', true);
            ...
            $this->session->set_userdata('email_queue', $email_data);
        }

        redirect($_SERVER['HTTP_REFERER']);
    }

    public function process_email_session()
    {
        $user_id = auth('id');
        // Get email queue from session
        $email_queue = $this->session->userdata('email_queue');

        if (!$email_queue) {
            echo json_encode(array('status' => 'error', 'message' => 'No emails to process.'));
            return;
        }
        ...
        foreach ($email_queue as &$email) {
            if ($email['status'] === 'pending') {
                $error = null;
                if ($this->user_email_m->promotion_mail($user_id, $email['email'], $email['subject'], $email['message'], $error)) {
                ...
                // Update session data
                $this->session->set_userdata('email_queue', $email_queue);

                $load_data = $this->load->view('backend/users/inc/mail_list', ['email_queue' => $email_queue], true);
                echo json_encode(array('status' => 'success', 'load_data' => $load_data));
                return;
            }
        }
        ...
        $this->session->unset_userdata('email_queue');
        $msg = '<div class="alert alert-success">
    <strong>' . __('success') . '</strong> ' . __('completed') . '
    </div>';
