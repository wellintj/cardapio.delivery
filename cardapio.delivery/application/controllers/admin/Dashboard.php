...
class Dashboard extends MY_Controller
{
	...
	public function index()
	{
		...
		if (isset($this->is_restautant) && $this->is_restautant == 1 && $this->auth['user_role'] == 0) {
	...
	public function add_packages()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$previous_price = $this->input->post('previous_price', TRUE);
			$order_types = $this->input->post('order_types', TRUE);
			$data = array(
...
----------------------------------------------*/
	...
	public function delete_user($id)
	{
		try {
			...
			$shop_id = $restaurant ? $restaurant->id : null;

			// 1. First delete user-related data
			$userTables = ['users_active_features', 'user_settings', 'allergens', 'menu_type'];
			$userDeleted = $this->deleteByUserID($id, $userTables);

			if (!$userDeleted) {
				throw new Exception('Failed to delete user-related data');
			}

			// 2. Then delete shop-related data if shop_id exists
			if ($shop_id) {
				$shopTables = [
				...
				$shopDeleted = $this->deleteByShopID($shop_id, $shopTables);

				if (!$shopDeleted) {
					throw new Exception('Failed to delete shop-related data');
				}
			}

			// 3. Delete restaurant record if shop_id exists
			if ($shop_id) {
				$restaurantDeleted = $this->admin_m->delete_all_by_user_id($id, 'restaurant_list');
				if (!$restaurantDeleted) {
					throw new Exception('Failed to delete restaurant record');
				}
			}

			// 4. Finally, delete the user record itself
			$userDeleted = $this->admin_m->delete($id, 'users');
			if (!$userDeleted) {
	...
	protected function deleteByShopID($shop_id, $tables)
	{
		try {
			foreach ($tables as $table) {
				$deleted = $this->admin_m->delete_all_by_vendor_id($shop_id, $table);
				if (!$deleted) {
					throw new Exception("Failed to delete records from table: {$table}");
				}
			}
			return true;
		} catch (Exception $e) {
	...
	protected function deleteByUserID($user_id, $tables)
	{
		try {
			foreach ($tables as $table) {
				$deleted = $this->admin_m->delete_all_by_user_id($user_id, $table);
				if (!$deleted) {
					throw new Exception("Failed to delete records from table: {$table}");
				}
			}
	...
	public function payment_by_admin($username, $status)
	{
		...
		if ($edit) :
			...
			$invoice_info = $this->admin_m->get_subscribed_package($user_info['user_id'], $package_info['id']);

			// if(isset($invoice_info->referal_code) && !empty($invoice_info->referal_code) && $invoice_info->is_payment==0){
			// 	$ref_code = $invoice_info->referal_code;
			// }else{
			// 	$ref_code = '';
			// }

			if (isset($invoice_info->amount) && !empty($invoice_info->amount) && $invoice_info->is_payment == 0) {
				__update_payment($user_info['user_id'], $package_info['id'], $invoice_info->amount);
			} else {
				__update_payment($user_info['user_id'], $package_info['id']);
			}

			//add payment info
			// $insert = $this->default_m->add_payment($user_info,$package_info,$payment_type=0,$ref_code,$is_payment=1);
	...
	public function transactions()
	{
		...
		$data['month'] = $this->admin_m->get_payment_income('month');
		$data['balance'] = $this->admin_m->get_payment_income('month');
		$data['total_income'] = $this->admin_m->get_payment_income('all');
	...
	public function add_email()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($id == 0) {
				$insert = $this->admin_m->__insert($data, 'email_template');
			} else {
				$insert = $this->admin_m->__update($data, $id, 'email_template');
			}
	...
	public function set_default($id)
	{
		...
		is_test();
		$this->db->update('packages', ['is_default' => 0]);
		$data = ['is_default' => 1];
		$edit = $this->admin_m->update($data, $id, 'packages');
		if ($edit) {
			redirect($_SERVER['HTTP_REFERER']);
		} else {
			$this->session->set_flashdata('error', 'Somthing worng. Error!!');
	...
	public function new_user()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($insert) {
				...
				$restaurant_id = $this->admin_m->insert($shop_data, 'restaurant_list');
				...
				$this->custom_m->add_default_data($restaurant_id, $insert);
	...
	public function get_expired_users()
	{
		...
		if (count($all_users) > 0) {
			foreach ($all_users as $users) {
				...
				$this->admin_m->update_by_user_id(['is_running' => 0], $users['id'], 'payment_info');
			}
		}
	}
	...
	public function summernote_image_upload()
	{
		if ($_FILES['file']['name']) {
			if (!$_FILES['file']['error']) {
				$name = md5(random_string('numeric', 10));
	...
	public function reset_qr()
	{
		...
		foreach ($users as $key => $user) {
			...
			$qr_image = $user['username'] . '_' . random_string('numeric', 10) . '.png';
		...
		$settings = settings();
		$this->load->library('ciqrcode');
		$qr_image = 'site_qr_' . random_string('numeric', 10) . '.png';
	...
	public function update_uids()
	{
		$item_list = $this->admin_m->select('items');
		$uid = time() . random_string('numeric', 3);
	...
	public function setEnvironment($value)
	{
		...
		if (!file_exists($envFilePath)) {
			$defaultEnvContent = "is_development={$en}\n";
	...
	public function malicious()
	{
		check_valid_auth();
		$data = array();
		$data['page_title'] = "Malicious";
		$data['page'] = "Malicious";
		$data['data'] = FALSE;
		$data['list'] = $this->admin_m->select('security_attempts');
		$data['main_content'] = $this->load->view('backend/admin_activities/malicious', $data, TRUE);
		$this->load->view('backend/index', $data);
	}

	public function mstatus($id, $status)
	{
		is_test();
		if (isset($_GET['a']) && $_GET['a'] == 'user') {
			$table = 'user_action_attempts';
		} else {
			$table = 'security_attempts';
		}
		$this->admin_m->update(['total_attempts' => $status], $id, $table);
	...
	public function add_ip()
	{
		...
		$this->form_validation->set_rules('ip_address', __('ip_address'), 'trim|xss_clean|required');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect($_SERVER['HTTP_REFERER']);
		} else {
			$app = isset($this->settings['attempt_config']) && isJson($this->settings['attempt_config']) ? json_decode($this->settings['attempt_config']) : '';
			$max_attempts = isset($app->spam_attempts) ? $app->spam_attempts : 3;
			...
			$insert = $this->admin_m->insert($data, 'security_attempts');
	...
	public function msettings()
	{
		...
		$data['data'] = FALSE;
		$data['settings'] = settings();
		$data['main_content'] = $this->load->view('backend/admin_activities/msettings', $data, TRUE);
		$this->load->view('backend/index', $data);
	}
	...
	public function user_attempts()
	{
		check_valid_auth();
		$data = array();
		$data['page_title'] = "User Attempts";
		$data['page'] = "Malicious";
		$data['data'] = FALSE;
		$data['list'] = $this->admin_m->select('user_action_attempts');
		$data['main_content'] = $this->load->view('backend/admin_activities/user_attempts', $data, TRUE);
		$this->load->view('backend/index', $data);
	}

	public function add_msettings()
	{
		is_test();
		if (check() != 1) :
			exit();
		...
		$this->form_validation->set_rules('spam_attempts', __('spam_attempts'), 'trim|xss_clean');
		$this->form_validation->set_rules('user_attempts', __('user_attempts'), 'trim|xss_clean');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('error', validation_errors());
			redirect($_SERVER['HTTP_REFERER']);
		} else {
			...
			$id = $this->input->post('id', TRUE);
			$data = [
				'is_spam' => $this->input->post('is_spam') ?? 0,
				'is_user_attempts' => $this->input->post('is_user_attempts') ?? 0,
				'spam_attempts' => $this->input->post('spam_attempts', true),
				'user_attempts' => $this->input->post('user_attempts', true),
			];


			$insert = $this->admin_m->update(['attempt_config' => json_encode($data)], $id, 'settings');
