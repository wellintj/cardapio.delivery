...
class Settings extends MY_Controller
{
	...
	protected function add_template()
	{
		$settings = $this->admin_m->get_settings('settings');
		if (empty($settings['email_template_config'])) {
			$this->load->model('custom_m');
			$tempData = $this->custom_m->getMailTepmlate();
			$this->admin_m->update(['email_template_config' => $tempData], $settings['id'], 'settings');
		};
	}
	...
	public function banner_settings()
	{
		...
		$data['settings'] = $this->admin_m->get_settings('settings');
		$data['site_banners'] = $this->admin_m->single_select('section_banners');
		$data['banners'] = $this->admin_m->select_ln('section_banners');
	...
	public function map_config()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'tax_system' => $this->input->post('tax_system', TRUE) ?? 'percentage',
			);
			if ($id == 0) {
				$insert = $this->admin_m->insert($data, 'settings');
			} else {
				$insert = $this->admin_m->__update($data, $id, 'settings');
			}
	...
	public function add_section_banner()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$is_clone = isset($_POST['is_clone']) && $_POST['is_clone'] == 1 ? 1 : 0;
			...
			if ($id == 0 || (isset($is_clone) && $is_clone == 1)) {
				...
				$check = $this->admin_m->get_single_section($section_name, $data['language']);
				if (!empty($check)) {
			...
			if ($check != 1) :
				...
				if ($insert) {
					...
					$this->session->set_userdata([
						'site_lang' => $data['language'],
						'frontend_lang' => $data['language'] ?? st()->language,
						'lang_vendor_id' => $id ?? null,
					]);
					$this->session->set_flashdata('success', !empty(lang('success_text')) ? lang('success_text') : 'Save Change Successful');
					redirect(base_url('admin/settings/settings'));
				} else {
	...
	public function add_email_content()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			endforeach;
			$insert = $this->admin_m->__update(['email_template_config' => json_encode($data)], $this->settings['id'], 'settings');
