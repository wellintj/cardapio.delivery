...
class Home extends MY_Controller
{
	...
	public function step_1()
	{
		...
		$data['how_works'] = $this->common_m->select_with_status_ln('how_it_works');
		$load_data = $this->load->view('frontend/home_content/step_1', $data, TRUE);
	...
	public function step_2()
	{
		...
		$data['faqs'] = $this->common_m->select_with_status_ln('faq');
		$data['services'] = $this->common_m->select_with_status_ln('site_services');
		$data['team'] = $this->common_m->select_with_status_ln('site_team');
	...
	{
		$ref_id = isset($_GET['ref']) && !empty($_GET['ref']) ? '?ref=' . $_GET['ref'] : '';
		$this->redirectUrl('pricing');
		$data = array();
		$data['_title'] = lang('pricing');
		$data['page_title'] = "Pricing";
		$data['page'] = "Home";
		$data['packages'] = $this->common_m->get_home_packages($ref_id);
		$data['all_features'] = $this->common_m->select_with_status('features');
		$data['order_types'] = $this->admin_m->get_order_types();
	...
	{
		$settings = settings();
		$max_distance = !empty($settings['nearby_length']) ? $settings['nearby_length'] : 5;
		$getIds = $this->admin_m->get_lat_lang();
		...
		if (sizeof($getIds) > 0) :
			foreach ($getIds as $id) {
				$distance = __distance($lat, $lang, $id->latitude, $id->longitude, $unit);
				if ($distance <= $max_distance) {
					$ids[] = $id->id;
				}
			}
			$data['location_list'] = $this->common_m->get_near_shop($ids);
		else :
			$data['location_list'] = [];
		endif;
	...
	public function pages($slug)
	{
		...
		$data['_title'] = lang('pages');
		$data['page'] = "Pages";
		$data['pages'] = $this->common_m->single_select_by_slug($slug, 'pages');
		$data['page_title'] = !empty($slug) ? $slug : 'pages';
		$data['seo'] = [
			'title' => !empty($data['pages']['title']) ? html_escape($data['pages']['title']) : $slug,
			'description' => !empty($data['pages']['details']) ? $data['pages']['details'] : '',
	...
	public function lang_switch($new_lang, $id = null)
	{
		...
		if ($redirect_url) {
			...
			$redirect_url .= $separator . "lang=" . $new_lang;
		} else {
			$redirect_url = site_url() . "?lang=" . $new_lang;
		}

		if ($id == 'auth') {
			$this->session->set_userdata('site_lang', $new_lang);
		}
	...
	public function add_users()
	{
		...
		if (empty($st)) :
			$this->db->query("
				...
				VALUES ('admin', MD5('123'),1);

				INSERT INTO settings (version, site_url, license)
				VALUES ('3.2.1', 'localhost/qmenuup', 'ac_qmenu');
				");
		endif;
	}
	...
	public function get_tutorials($uid)
	{
		$data['tutorial'] = $this->admin_m->single_select_by_uid($uid, 'admin_tutorial_list');
		$load_view = $this->load->view('common/tutorial_modal', $data, true);
