	...
	<div class="row no-gutters">
		<?php foreach ($specialties as $key1=> $row): ?>
		  <div class="col-lg-6 col-sm-6 q-sm">
		    <div class="homeSinglePackage w-50p">
		        ...
		        <div class="text <?= in_array($key1+1, $item_array)?"order-last":"order-first" ;?>">
		          ...
		          <div class="port_d_flex home_view mt-10 specialiatiesButton">
		            <a href="javascript:;" class="quick_view" data-type='specialties'  data-id="<?=  html_escape($row['id']);?>" data-placement="top" data-toggle=""  title="<?= lang('quick_view'); ?>"><i class="icofont-eye-open"></i> <?= lang('details'); ?></a>
		            <?php if(shop($row['shop_id'])->is_cart == 1): ?>

			            <?php if(shop($row['shop_id'])->stock_status == 1): ?>
			            	<?php if($row['in_stock'] > $row['remaining']): ?>
					            <a href="javascript:;" class="btn custom-btn add_to_cart"  data-id="<?=  html_escape($row['id']);?>" data-type="package" data-placement="top" data-toggle="tooltip" title="<?= lang('add_to_cart'); ?>"><i class="icofont-ui-cart"></i> <?= lang('order_now'); ?></a>
					        <?php endif;?>
					    <?php else: ?>
			            	<a href="javascript:;" class="btn custom-btn add_to_cart"  data-id="<?=  html_escape($row['id']);?>" data-type="package" data-placement="top" data-toggle="tooltip" title="<?= lang('add_to_cart'); ?>"><i class="icofont-ui-cart"></i> <?= lang('order_now'); ?></a>
			        	<?php endif;?>
