<?php foreach ($this->cart->contents() as $key => $row): ?>
	<?php $shop_id = $row['shop_id']??0; ?>
	<?php if(!isset($row['is_pos']) || $row['is_pos'] == 0): ?>
	<li>
		<div class="single_item_cart">
			<div class="incress_area">
				...
				<span class="cart_qty_field"><input readonly type="text" id="qty_<?= $row['rowid'] ;?>" class="qty" value="<?= $row['qty'] ;?>" min-value='1' /></span>
				...
				<div class="single_cart_item_details">
					...
					<div class="sizePrice">
						...
						<?php if((isset($row['is_variants']) && $row['is_variants']==1) && (isset($row['sizes']['size_slug']) && !empty($row['sizes']['size_slug']))): ?>
							...
							<p class="sizeTagName f-color"><?= isset($item_size) && !empty($item_size)?$item_size->variant_name.': '.$item_size->name:'';?></p>
						<?php else: ?>
							<p><?= isset($row['sizes']['size_slug']) && !empty($row['sizes']['size_slug'])?lang('size').' : '.get_size($row['sizes']['size_slug'],$row['shop_id']):"" ;?></p>
						<?php endif; ?>
					</div>
						
					<p class="currencyTag"><?=  currency_position($row['price'],$row['shop_id']).' x '.html_escape($row['qty']) ;?> = <?=currency_position(($row['price']*$row['qty']),$row['shop_id']);?></p>
