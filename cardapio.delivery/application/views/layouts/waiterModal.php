	      ...
	      <form action="<?= base_url('profile/call_waiter') ;?>" class="defaultForm callWaiterForm">
		      ...
		      <div class="modal-body">
		        ...
		        <div class="form-group">
		        	<?php 
		        			 if (!empty(temp('table_no')) && temp('table_no')) : 
		        					$tableNo = temp('table_no');
		        			endif;
		        	 ?>
		        	<select name="table_no" id="table_no" class="form-control">
		        		...
		        		<?php foreach ($table_list as $key => $table): ?>
			        				...
			        				<?php elseif(isset($tableNo)): ?>
			        						<option value="<?=  $table['id'];?>" <?= isset($tableNo) && $tableNo==$table['id']?"selected":'' ;?>><?= $table['name']; ?></option>
