...
<form action="<?= base_url('customer/update_customer'); ?>" method="post" class="serviceLogin">
	...
	<div class="modal-body">
		...
		<?php if ((isset($customer_data['is_email_based']) && $customer_data['is_email_based'] == 1) || auth('is_email_based')) : ?>
			<div class="form-group">
				<label for=""><?= lang('email'); ?> </label>
				<input type="email" name="customer_email" value="<?= isset($customer_data['customer_email']) ? $customer_data['customer_email'] : auth('customer_email'); ?>" class="form-control">
			</div>
		<?php endif; ?>
		<div class="form-group">
		...
		<?php $get_q = isset($customer_data['question'])  && isJson($customer_data['question']) ? json_decode($customer_data['question']) : json_decode(auth('question')); ?>
