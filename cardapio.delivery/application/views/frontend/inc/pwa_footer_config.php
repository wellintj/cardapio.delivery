<?php if (isset($page) && $page != 'Profile' && !isset($slug)) : ?>
	...
	<?php if (isset($settings['is_pwa']) && $settings['is_pwa'] == 1 && isset($pwa2->is_pwa_active) && $pwa2->is_pwa_active == 1) : ?>
		<div class="add-to-2">
			<div class="add-to-area">
				...
				<div class="pwaContent">
					<img src="<?= asset_url(!empty($pwa2->logo) ? $pwa2->logo : "uploads/pwa/logo.png"); ?>" alt="">
		...
		<script type="text/javascript">
			...
			if ('serviceWorker' in navigator) {
				navigator.serviceWorker.register(base_url + 'public/service-worker-2.js?r=', {
					scope: `${base_url}public/`
		...
		<?php if (is_feature($u_info['id'], 'pwa-push') == 1 && is_active($u_info['id'], 'pwa-push')) : ?>
			<div class="add-to">
				<div class="add-to-area">
					<a href="javascript:;" class="pwaClose"><i class="fa fa-close"></i></a>
					<div class="pwaContent">
						<img src="<?= asset_url(!empty($pwa->logo) ? $pwa->logo : "uploads/pwa/avatar.png"); ?>" alt="">
						<div class="pwaDetails">
							<h4><?= $pwa->title; ?></h4>
							<p><?= asset_url($slug); ?></p>
			...
			<script type="text/javascript">
				...
				if ('serviceWorker' in navigator) {
					navigator.serviceWorker.register(`${base_url}public/service-worker.js?slug=${slug}`, {
						scope: `${base_url}public/`
					}).then(function(registration) {
						// Registration was successful
						console.log('PWA: ServiceWorker registration successful with scope: ', registration.scope);
