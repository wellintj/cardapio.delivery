            ...
            <div class="princing-item red bg-white mt-0 shadow">
                <div class="priceTopCard">
                    <div class="pricing-divider ">
                        ...
                        <?php else: ?>
                            <h4 class="my-0  text-light font-weight-normal mb-3"><?= !empty($package['previous_price']) ? "<span class='previous_price fz-14 text-light '>" . admin_currency_position($package['previous_price']) . "</span>" : ""; ?> <?= admin_currency_position(html_escape($package['price'])); ?> <span class="h5"> / <?= !empty($package['package_type']) ? get_package_type($package['package_type'], $package['duration'], $package['duration_period']) : get_package_type($package['package_type'], $package['duration'], $package['duration_period']); ?></span></h4>
                        <?php endif; ?>
                        ...
                        <ul class="list-unstyled position-relative">
                            <?php foreach ($all_features as $key2 => $feature): ?>
                                ...
                                <?php else: ?>
                                    ...
                                    <li><i class="fas <?= isset($feature_id['feature_id']) && $feature_id['feature_id'] == $feature['id'] ? 'fa-check c_green' : 'fa-times c_red'; ?> "></i> <?= change_name($feature['slug']); ?> <?= html_escape($feature['slug']) == 'menu' ? ' <b>(' . limit_text($package['item_limit']) . ' ' . lang('items') . ') </b>' : ''; ?> <?= html_escape($feature['slug']) == 'order' ? ' <b>(' . limit_text($package['order_limit']) . ') </b>' : ''; ?></li>
                                ...
                                <fieldset>
                                    ...
                                    <ul class="mt-0 pl-0 ml-0 fa-ul order_type_list">
                                        ...
                                        <?php foreach ($order_types as  $key => $order): ?>
                                            <li><i class="fas <?= in_array($order['id'], $order_list) ? 'fa-check c_green' : 'fa-times c_red'; ?> "></i> <?= !empty(__($order['slug']))?__($order['slug']):$order['name']; ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                </fieldset>
                            </div>
                ...
                <div class="priceCardFooter">
                    <a href="<?= base_url('signup-' . $package['slug']) . $ref_id; ?>" class="btn btn-lg btn-block btn-custom "><?= !empty(lang('select_package')) ? lang('select_package') : 'Select Package'; ?></a>
                </div>
            </div>
        <?php endforeach; ?>
