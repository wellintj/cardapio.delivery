...
<div class="home_page_navbar" id="headers">
    ...
    <nav class="navbar navbar-expand-lg  homeMenuNav">
        <div class="container">
            <a class="navbar-brand" href="<?= base_url(); ?>"><img src="<?= asset_url(!empty($settings['logo']) ? $settings['logo'] : "assets/frontend/images/logo-example.png"); ?>" class="logo" alt=""></a>
                ...
                <div class="custom_menu">
                    <ul class="navbar-nav">
                        <li class="nav-item <?= isset($page_title) && $page_title == 'Home' ? 'active' : ''; ?>">
                            <a class="nav-link" href="<?= base_url_with_ref(); ?>"><?= !empty(html_escape(lang('home'))) ? html_escape(lang('home')) : "Home"; ?>
                        ...
                        <?php if (!empty(section_name('pricing')) && section_name('pricing')['status'] == 1 && $settings['is_registration'] == 1) : ?>
                            <li class="nav-item <?= isset($page_title) && $page_title == 'Pricing' ? 'active' : ''; ?>">
                                <a class="nav-link" href="<?= base_url_with_ref('pricing'); ?>"><?= !empty(html_escape(lang('pricing'))) ? html_escape(lang('pricing')) : "pricing"; ?></a>
                            </li>
                        <?php endif; ?>
                        <?php if (!empty(section_name('reviews')) && section_name('reviews')['status'] == 1) : ?>
                            <?php if ($this->common_m->total_rating() > 0) : ?>
                                <li class="nav-item <?= isset($page_title) && $page_title == 'Reviews' ? 'active' : ''; ?> hidden">
                                    <a class="nav-link" href="<?= base_url_with_ref('reviews'); ?>"><?= !empty(html_escape(lang('reviews'))) ? html_escape(lang('reviews')) : "reviews"; ?></a>
                                </li>
                            <?php endif; ?>
                        <?php endif; ?>
                        ...
                        <?php if ($settings['is_user'] == 1) : ?>
                            <?php if ($this->common_m->count_total_user() > 0) : ?>
                                <li class="nav-item <?= isset($page_title) && $page_title == 'Users' ? 'active' : ''; ?>">
                                    <a class="nav-link" href="<?= base_url_with_ref('users'); ?>"><?= !empty(html_escape(lang('users'))) ? html_escape(lang('users')) : "users"; ?></a>
                                </li>
                            <?php endif; ?>
                        <?php endif; ?>
                        ...
                        <?php if (isset($settings['restaurant_demo'])  && !empty($settings['restaurant_demo']) == 1) : ?>
                            <li class="nav-item <?= isset($page_title) && $page_title == 'Demo' ? 'active' : ''; ?>">
                                <a class="nav-link" href="<?= asset_url($settings['restaurant_demo']); ?>"><?= lang('demo'); ?></a>
                            </li>
                        <?php endif; ?>
                        ...
                        <?php if (!empty(section_name('contacts')) && section_name('contacts')['status'] == 1) : ?>
                            <li class="nav-item <?= isset($page_title) && $page_title == 'Contacts' ? 'active' : ''; ?>">
                                <a class="nav-link" href="<?= base_url_with_ref('contacts'); ?>"><?= !empty(html_escape(lang('contacts'))) ? html_escape(lang('contacts')) : "contacts"; ?></a>
                            </li>
                        <?php endif; ?>
                                ...
                                <?php if (sizeof($languages) > 1) : ?>
                                    <li class="nav-item dropdown">
                                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            ...
                                            <?= lang_slug(!empty(site_lang()) ? site_lang() : $settings['language']); ?>
                                        </a>
                                        <div class="dropdown-menu languageDropdown" aria-labelledby="navbarDropdown">

                                            <?php foreach ($languages as $key => $language) : ?>
                                                <a class="dropdown-item" href="<?= base_url_with_ref('home/lang_switch/' . $language['slug'] . '/auth'); ?>"><?= $language['lang_name']; ?></a>
                                            <?php endforeach ?>
                        ...
                        <div class="login_signup">
                            <?php if ($settings['is_registration'] == 1) : ?>
                                <?php if ($settings['is_signup'] == 1) : ?>
                                    ...
                                    <?php if (empty($ref_id)) : ?>
                                        <a href="<?= base_url_with_ref('sign-up'); ?>"><i class="fa fa-user-plus"></i>
                                            &nbsp;<?= !empty(html_escape(lang('sign-up'))) ? html_escape(lang('sign-up')) : "Signup"; ?></a>
                                    <?php else : ?>
                                        <a href="<?= base_url_with_ref('pricing'); ?>"><i class="fa fa-user-plus"></i>
                                            &nbsp;<?= !empty(html_escape(lang('sign-up'))) ? html_escape(lang('sign-up')) : "Signup"; ?></a>
                                    <?php endif; ?>
