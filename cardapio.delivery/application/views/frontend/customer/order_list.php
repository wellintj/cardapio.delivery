        ...
        <div class="row">
            ...
            <div class="col-lg-9 col-12">
                <div class="serviceRightSide">
                                ...
                                <tbody>
                                    <?php foreach ($order_list as $key => $row) : ?>
                                        <tr>
                                            ...
                                            <td data-label="<?= lang('order_id'); ?>" class="orderTag">#<?= $row['uid']; ?></td>
                                            <td data-label="<?= lang('shop_name'); ?>"><?= shop($row['shop_id'])->name; ?></td>
                                            <td data-label="<?= lang('order_type'); ?>">
                                                <?= ucfirst(order_type($row['order_type'])); ?>

                                            </td>
                                            <td data-label="<?= lang('total_qty'); ?>"><?= $row['total_item']; ?></td>

                                            <td data-label="<?= lang('total_price'); ?>" class="text-center">
                                                <p class="priceTag"><?= currency_position($row['total'], $row['shop_id']); ?>
                                                </p>
                                                <?php if ($row['order_type'] == 1 || $row['order_type'] == 5) : ?>
                                                    <p class="fz-13 badge"><?= lang('shipping'); ?> :
                                                        <?= $row['delivery_charge'] == 0 ? lang('free') : currency_position($row['delivery_charge'], $row['shop_id']); ?>
                                                    </p>
                                                <?php endif; ?>
                                            </td>
                                            <td data-label="<?= lang('date'); ?>">
                                                <label class="badge badge-secondary">
                                                    <?= full_time($row['created_at'], $row['shop_id']); ?></label>
                                                <?php if ($row['order_type'] == 4) : ?>
                                                    <p class="badge"><?= lang('pickup_time'); ?>
                                                        <?= !empty($row['pickup_time']) ? $row['pickup_time'] : time_format_12($row['reservation_date']); ?>
                                                    </p>
                                                <?php endif; ?>
                                            </td>
                                                ...
                                                <div class="">
                                                    ...
                                                    <?php elseif ($row['status'] == 2) : ?>
                                                        ...
                                                        <?php if ($row['order_type'] == 1 && $row['is_db_completed'] == 0) : ?>
                                                            <a href="javascript:;" class="badge badge-success"><i class="icofont-check-alt"></i></i>
                    ...
                    <div class="ci-paginationArea mt-20 text-center">
                    ...
                    <div class="modal-body">
                        <div class="row justify-content-center">
                            <div class="form-group text-center ratingArea">
                                <h5 class="text-center fz-16 mb-3">
                                    <?= !empty(lang('give_your_feedback')) ? lang('give_your_feedback') : "Please give your feedback"; ?>
                                </h5>
