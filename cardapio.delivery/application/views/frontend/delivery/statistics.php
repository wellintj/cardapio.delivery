                    ...
                    <div class="searchArea">
                        <form action="<?= base_url("staff/report");?>" method="get">
                            ...
                            <div class="custom_fields">
                                <a href="<?= base_url("staff/report?q=all")?>" class="btn btn-secondary"
                                    style="margin-right:10px;"><?= lang('all');?></a>
                                <select name="m" class="form-control" onchange="this.form.submit()">
                                    <?php foreach ($dboy_date['month'] as $month): ?>
                                    <?php if(isset($_GET['m'])): ?>
                                    <option value="<?= $month;?>"
                                        <?= isset($_GET['m']) && $_GET['m']==$month?"selected":"";?>><?= $month;?>
                                    </option>
                                    <?php else: ?>
                                    <option value="<?= $month;?>" <?=  date('m')==$month?"selected":"";?>><?= $month;?>
                                    </option>
                                    <?php endif ?>
                                ...
                                <select name="y" class="form-control" onchange="this.form.submit()">
                                    ...
                                    <?php if(isset($_GET['y'])): ?>
                                    <option value="<?= $year;?>"
                                        <?= isset($_GET['y']) && $_GET['y']==$year?"selected":"";?>>
                                        <?= !isset($_GET['y'])?date('Y'):$year;?></option>
                                    <?php else: ?>
                        ...
                        <table class="table table-striped table-condensed">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= currency_position($row['total'],$row['shop_id']);?></td>
