					...
					<div class="profleForm">
						...
						<form action="<?= base_url('staff/update_delivery_profile'); ?>" method="post" enctype="multipart/form-data">
							...
							<div class="row">
								...
								<?php if (isset(st()->is_city) && st()->is_city == 1): ?>
									<div class="form-group col-md-6">
										<label for=""><?= lang('city_name'); ?></label>
										<select name="city_id" id="city_id" class="form-control">
											<option value=""><?= lang('select'); ?></option>
											<?php foreach ($cityList as $key => $city) : ?>
												<option <?= isset($info['city_id']) && $info['city_id'] == $city['id'] ? "selected" : ""; ?> value="<?= $city['id']; ?>"><?= $city['city_name']; ?> <?= !empty($city['state']) ? ' - ' . $city['state'] : '' ?></option>
											<?php endforeach ?>
										</select>
