...
<?php if (isset($slider_list) && sizeof($slider_list) > 0) : ?>
	<?php include VIEWPATH . "common_layouts/home_slider.php"; ?>
<?php else : ?>
	<?php if (isset($user_settings['is_banner']) && $user_settings['is_banner'] == 0) : ?>
		<section class="user_home_banner bmenu_style_<?= isset($user['menu_style']) ? $user['menu_style'] : 0 ?> img bg_loader" data-src="<?= base_url(!empty(restaurant($id)->cover_photo) ? restaurant($id)->cover_photo : 'assets/frontend/images/bg/bg_1.jpg'); ?>" style="background: url(<?= bg_loader(); ?>);">
			<div class="userHomebanner_wrapper">
				<div class="row">
					<div class="col-lg-12">
						<div class="homeBanner_left">
							<div class="shopLogo <?= imgRation(restaurant($id)->thumb ?? '') ?>">
							...
							<div class="homeBanner_btn">
								<h4 class="shopName"><?= !empty(restaurant($id)->name) ? restaurant($id)->name : restaurant($id)->username; ?></h4>
							...
							<div class="homeBanner_Service">
								<ul>
									<?php if (!empty($u_settngs['icon_settings']) && isJson($u_settngs['icon_settings']) > 0) : ?>
				...
				<div class="userHome_social">
					<ul>
						...
						<?php if (isset($social['tiktok']) && !empty($social['tiktok'])) : ?>
