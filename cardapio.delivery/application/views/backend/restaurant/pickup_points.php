							...
							<div class="row p-15">
								<div class="table-responsive">
									<div class="pickup-slots">
										<?php $pickup_slots = isJson($restaurant['pickup_time_slots'])?json_decode($restaurant['pickup_time_slots'],true):''; ?>
										...
										<label class="time_slot success-light <?= !empty($pickup_slots) && !empty($timeSlot) && in_array($timeSlot,$pickup_slots)==1?"active":"" ;?>">

											<input type="checkbox" class="timeCheckeds" name="time_slots[<?= $key2;?>]" value="<?= $timeSlot ;?>" <?= isset($pickup_slots) && !empty($pickup_slots)  && in_array($timeSlot,$pickup_slots)==1?"checked":"" ;?>> <?= time_format($slots['slot_start_time'],restaurant()->id) ;?> - <?= time_format($slots['slot_end_time'],restaurant()->id) ;?>
										</label>
