<div class="row resoponsiveRow">	
	...
	<div class="col-md-6">
		<form action="<?= base_url('admin/auth/add_pwa') ;?>" method="post" class="validForm" enctype= "multipart/form-data">
			<input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>">
			<div class="card pwa">
				<div class="card-header"><h4><?= lang('pwa'); ?></h4></div>
				<div class="card-body">
					<?php $pwa = !empty($settings['pwa_config'])?json_decode($settings['pwa_config']):''; ?>
					<div class="row">
							...
							<div class="form-group ">
								...
								<div class="">
									<label class="image_upload_box logo pwa">
										<img src="<?= !empty($pwa->logo)?base_url($pwa->logo):base_url('assets/frontend/images/avatar.png'); ?>" class="service_icon_preview" alt="">
						...
						<div class="form-group col-md-12">
							...
							<input type="text" name="title" class="form-control" value="<?= !empty($pwa->title)? $pwa->title:auth('username');?>" required>
						...
						<div class="col-md-12">
							<div class="form-group">
								<label class="custom-radio mr-20"> <input type="radio" name="is_pwa_active"   <?= isset($pwa->is_pwa_active) && $pwa->is_pwa_active==1?"checked":"";?> value="1" required><?= lang('active')?></label>

								<label class="custom-radio"> <input type="radio" name="is_pwa_active" value="0" <?= isset($pwa->is_pwa_active) && $pwa->is_pwa_active==0?"checked":"";?>  required><?= lang('disable')?></label>
							</div>
						</div>
