			...
			<form action="<?= base_url('admin/auth/add_features_info'); ?>" method="post">
				<div class="box-body">
					<div class="upcoming_events">
						...
						<?php $activeFeatures = ['welcome', 'menu', 'packages', 'specialities', 'reservation', 'contacts']; ?>
						<div class="row">
							<div class="">
								<?php foreach ($pricing as $key => $row) : ?>
									<?php foreach ($features as $key => $feature) : ?>
										...
										<?php if (isset($feature_id['feature_id']) && $feature_id['feature_id'] == $feature['feature_id']) : ?>
											<div class="col-sm-6 <?= site_lang() != 'english' && $feature['is_header']==0?"hidden":"" ?>">
												<div class="box <?= $feature['is_header']==1?"box-info":"" ?> ">
													<div class="box-header with-border">
														<h3 class="box-title"><?= html_escape($feature['name']); ?></h3>
														<?php if (site_lang() == 'english') : ?>
															<div class="box-tools pull-right">
																<div class="gap">
																	<input type="checkbox" id="toggle_<?= html_escape($feature['id']); ?>" name="set-name" class="switch-input toggle_feature" data-value="<?= html_escape($feature['status']); ?>" data-id="<?= html_escape($feature['feature_id']); ?>" data-lang="<?= site_lang(); ?>" <?= isset($feature['status']) && html_escape($feature['status']) == 1 ? 'checked' : ''; ?>>

																	<label for="toggle_<?= html_escape($feature['id']); ?>" class="switch-label pt-0 mt-3"> <span class="toggle--on"> <i class="fa fa-check c_green"></i> <?= !empty(lang('on')) ? lang('on') : "On"; ?></span><span class="toggle--off"><i class="fa fa-ban c_red"></i> <?= !empty(lang('off')) ? lang('off') : "Off"; ?></span></label>
																</div>
															</div>
														<?php endif; ?>
													</div>
													<!-- /.box-header -->
													<div class="box-body <?= $feature['is_header']==0?"hidden":"" ?>">
