		...
		<form action="<?= base_url("admin/auth/add_whatsapp_message_config"); ?>" method="post">
			...
			<div class="card">
				<div class="card-header">
					<h3 class="title"><?= lang('whatsapp_message_for_order_status'); ?></h3>
				</div>
				<div class="card-body">
					<div class="form-group">
						<select name="msg_id_type" class="form-control">
							...
							<option value="wazfy" <?= isset($msg->msg_id_type) && !empty($msg->msg_id_type) && $msg->msg_id_type == "wazfy" ? "selected" : ''; ?>>Wazfy</option>
						</select>
					</div>
					...
					<div class="whatsappMessageArea">
						<div class="form-group mt-10">
							<label class="custom-checkbox"><input type="checkbox" class="msgCheckbox" data-id="welcome_msg" name="is_welcome" value="1" <?= isset($msg->is_welcome) && $msg->is_welcome == 1 ? "checked" : ''; ?>><?= lang('order_placed_message'); ?></label>
							<div class="mb-5">
								<code>{CUSTOMER_NAME}, {ORDER_ID}, {SHOP_NAME}, {TRACK_URL}</code>
							</div>
							<div class="welcome_msg <?= isset($msg->is_welcome) && $msg->is_welcome == 1 ? "" : 'dis_none'; ?>">
								<textarea name="welcome_msg" id="welcome_msg" class="form-control" cols="5" rows="5"><?= !empty($msg->welcome_msg) ? json_decode($msg->welcome_msg) : ''; ?></textarea>
							</div>
						</div>
