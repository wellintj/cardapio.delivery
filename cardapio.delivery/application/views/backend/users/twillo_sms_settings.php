			...
			<div class="card">
				...
				<div class="card-body">
					<div class="row">
						...
						<div class="form-group col-md-12">
							<label for="price"><?= !empty(lang('order_placed_message')) ? lang('order_placed_message') : "Order placed Message"; ?> <i class="fa fa-info-circle" data-target="#hintsModal" data-toggle="modal" title="How to set message"></i>
							</label>
							<div class="mb-5">
								<code>{SHOP_NAME}, {USER_NAME}, {ORDER_ID}</code>
							</div>
							<textarea name="welcome_msg" id="welcome_msg" cols="5" rows="5" class="form-control"><?= isset($data['welcome_msg']) ? json_decode($data['welcome_msg']) : ""; ?></textarea>
						</div>
						...
						<div class="form-group col-md-12">
							<label for="otp_message"><?= !empty(lang('otp_message')) ? lang('otp_message') : "OTP Message"; ?> <i class="fa fa-info-circle" data-target="#hintsModal" data-toggle="modal" title="How to set message"></i></label>
							<div class="mb-5">
								<code>{SHOP_NAME}, {OTP}</code>
							</div>
							<textarea name="otp_msg" id="otp_message" cols="5" rows="5" class="form-control"><?= isset($data['otp_msg']) ? json_decode($data['otp_msg']) : ""; ?></textarea>
						</div>


						<div class="form-group col-md-6">
							<label for="price"><?= !empty(lang('order_placed_message')) ? lang('order_placed_message') : "Order Placed Message"; ?></label>
							<div class="">
								<select name="is_welcome_msg" id="is_sms" class="form-control">
									<option value="1" <?= isset($data['is_welcome_msg']) && $data['is_welcome_msg'] == 1 ? "selected" : ""; ?>><?= lang('active'); ?></option>
									<option value="0" <?= isset($data['is_welcome_msg']) && $data['is_welcome_msg'] == 0 ? "selected" : ""; ?>><?= lang('inactive'); ?></option>
								</select>
							</div>
						</div>
