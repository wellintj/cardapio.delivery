				...
				<div class="box-tools pull-right">
					<?php if (isset($active) && $active == 1): ?>
						<?php if (is_access('add') == 1): ?>
							<a href="<?= base_url('admin/menu/create_packages?lang=' . site_lang()); ?>" class="btn btn-secondary "><i class="fa fa-plus"></i> &nbsp;<?= !empty(lang('add_new')) ? lang('add_new') : "Add New"; ?> </a>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			</div>
							...
							foreach ($packages as $key => $value): ?>
								<?php if (count($value['items']) > 0): ?>
									<tr>
										...
										<td class="bg_white sm-pl-0 sm-mt-10">
													...
													<tbody>
														...
														foreach ($value['items'] as $row): ?>
															<tr>
																...
																<td data-label="<?= lang('title'); ?>"><?= html_escape($row['title']); ?></td>
																<td data-label="<?= lang('price'); ?>"><?= currency_position($row['item_price'], restaurant()->id); ?></td>
															</tr>
														<?php $j++;
														endforeach ?>
													</tbody>
										...
										<td data-label="<?= lang('action'); ?>" class="action">
											<a href="<?= base_url('admin/menu/edit_packages/' . html_escape($value['package_id'])); ?>?action=clone" class="btn btn-secondary btn-sm"><i class="fa fa-clone"></i></a>
											<div class="btn-group">
