<div class="row">
	<div class="col-md-6">
		<?php
		$total = $this->admin_m->count_packages_user_id('item_packages_list', $is_special = 0);
...
if (isset($_GET['action']) && $_GET['action'] == "clone") {
	$url = base_url('admin/menu/add_packages?action=clone');
} else {
	$url = base_url('admin/menu/add_packages');
}
?>
	...
	<div class="col-lg-6">
		<div class="box box-primary">
			<div class="box-header with-border">
				<h3 class="box-title"><?= !empty(lang('add_packages')) ? lang('add_packages') : "Add Package"; ?> - <small class="text-purple fz-14"><i class="fa fa-language"></i> <?= __language($data ?? ''); ?></small></h3>
				...
				<div class="box-body">
						...
						<div class="form-group col-md-12">
							...
							<select name="items[]" class="form-control multiselct" multiple="" id="">
								...
								<?php foreach ($items as $key => $item) : ?>
									<option <?= isset($data['item_id']) && is_array(json_decode($data['item_id'])) && in_array($item['id'], json_decode($data['item_id'])) == 1  ? "selected" : ""; ?> value="<?= $item['id']; ?>" <?= isset($data['package_type']) && html_escape($data['package_type']) == $item['id'] ? 'selected' : ''; ?>><?= $item['title']; ?> / <?= restaurant()->icon; ?><?= $item['price']; ?></option>
								<?php endforeach ?>
							</select>
						</div>
					</div>
					...
					<div class="row">
						<div class="form-gorup col-md-12 mb-5">
							<label for="price"><?= !empty(lang('price')) ? lang('price') : "Price"; ?>
								<label data-toggle="tooltip" data-placement="top" title="<?= __('if_you_want_to_set_custom_price') ?>" class="pointer p-5 btn label-info discount_label label_input mr-15"><input type="checkbox" name="is_price" class="is_price" value="1" <?= isset($data['is_price']) && $data['is_price'] == 1 ? 'checked' : '';; ?>> &nbsp; <?= lang('is_price'); ?></label>

								<label class="pointer p-5 btn label-primary discount_label label_input"><input type="checkbox" name="is_discount" class="is_discount" value="1" <?= isset($data['is_discount']) && $data['is_discount'] == 1 ? 'checked' : '';; ?>> &nbsp;<?= lang('is_discount'); ?></label>
					...
					<div class="row">
						<div class="form-group col-md-12">
							...
							<textarea name="details" id="" class="data_textarea form-control" cols="30" rows="10"><?= !empty($data['details']) ? html_escape($data['details']) : set_value('details'); ?></textarea>
						</div>
					</div>
					<div class="row">
						<div class="form-group col-md-12">
