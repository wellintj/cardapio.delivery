...
<header class="main-header">
  ...
  <?php else : ?>
    <a href="<?= isset($page) && $page == "POS" ? base_url('dashboard') : base_url('dashboard'); ?>" class="logo shopLogo">
      <!-- mini logo for sidebar mini 50x50 pixels -->
      <span class="logo-mini"><b></b></span>
      <!-- logo for regular state and mobile devices -->
      <span class="mini-logo">
    ...
    <div class="rightMenuBar">
      ...
      <?php if ($auth_info['user_role'] == 0) : ?>
        <a href="<?= url(html_escape($this->my_info['username'])); ?>" class="navLink" target='blank'>
          <i class="fa fa-eye"></i> <span class="hidden-xs"><?= !empty(lang('view_shop')) ? lang('view_shop') : "View Shop"; ?></span>
        </a>
      <?php endif; ?>
    </div>
      ...
      <ul class="nav navbar-nav">
        <?php if (auth('user_role') == 0) : ?>
          ...
          <?php if (isset($glang['is_glang']) && $glang['is_glang'] == 1) : ?>
            <li class="gLanguage p-r">
              <div class="gtranslate_wrapper gtranslator"></div>
            </li>
          <?php else : ?>
            <?php $shop_languages = shop_languages(auth('id')); ?>
            <?php if (count($shop_languages) > 1) : ?>
              <li class="dropdown">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                  <i class="icofont-globe"></i>
                  <?= lang_slug(!empty(site_lang()) ? site_lang() : $settings['language']); ?> &nbsp;<span class="caret"></span></a>
                <ul class="dropdown-menu" role="menu">
                  <?php foreach ($shop_languages as $key => $language) : ?>
                    <li>
                      <a class="dropdown-item" href="<?= base_url('home/lang_switch/' . $language->slug . '/auth'); ?>"> <?= $language->lang_name; ?></a>
                    </li>
                  <?php endforeach ?>
        ...
        <?php else : ?> <!-- user_role -->
          ...
          <?php if (isset($settings['language_type']) && $settings['language_type'] == 'google') : ?>
            <li class="gLanguage p-r">
              <div class="admin_gtranslate_wrapper gtranslator"></div>
            </li>
          <?php else : ?>
        ...
        <li class="dropdown user user-menu">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
            <img src="<?= avatar(!empty($auth_info['thumb']) ? $auth_info['thumb'] : '', 'profile') ?>" class="user-image uploaded_img" alt="User Image">
            <span class="hidden-xs"><?= isset($auth_info['name']) && !empty($auth_info['name']) ? $auth_info['name'] : $auth_info['username']; ?></span>
          </a>
          <ul class="dropdown-menu">
            <!-- User image -->
            <li class="user-header">
              <img src="<?= avatar(!empty($auth_info['thumb']) ? $auth_info['thumb'] : '', 'profile') ?>" class="img-circle uploaded_img" alt="User Image">
