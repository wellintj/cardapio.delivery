								...
								<tbody>
									...
									<?php foreach ($all_item_list as $key => $row): ?>
										<?php 
							             ...
							             $tips =$row['tips'];
							             $grand_total = grand_total($net_total,$row['delivery_charge'],$row['discount'],$row['tax_fee'],$row['coupon_percent'],$row['tips'],$row['order_type'],$tax_status);
										...
										<tr>
											...
											<td>
												...
												<?php if($row['is_size']==1): ?>
													<span class="badge sizeTag"><?= isset($row['size_slug'])? variants($row['size_slug'],$row['item_id'],$row['shop_id']):'';?></span>
												<?php endif;?>
												</p>
