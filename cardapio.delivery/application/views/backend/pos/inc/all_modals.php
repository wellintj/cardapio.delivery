<!-- 
 ...
 <!-- 
	Shipping Modal
 -->
 <div id="serviceModal" class="modal fade discountModal" role="dialog" data-backdrop="static" data-keyboard="false">
 	<div class="modal-dialog modal-sm">

 		<form action="<?= base_url('admin/pos/set_service'); ?>" method="post" class="setDiscount validForm">
 			<?= csrf();?>
 			<!-- Modal content-->
 			<div class="modal-content">
 				<div class="modal-header">
 					<button type="button" class="close" data-dismiss="modal">&times;</button>
 					<h4 class="modal-title"><?= lang('servie_charge'); ?></h4>
 				</div>
 				<div class="modal-body custom-fields">
 					<div class="errorMsg"></div>
 					<div class="row">
 						<div class="form-group col-md-12">
 							<div class="input-group">
 								<input type="text" name="service_charge" class="form-control" placeholder="<?= lang('servie_charge');?>" value="<?= isset($service_charge)?$service_charge:"";?>" min-value="0" onkeypress='return event.charCode == 46 || (event.charCode >= 48 && event.charCode <= 57)' >
 									<span class="input-group-addon"><?= restaurant()->icon;?></span>
 								</div>
 							</div>
 						</div><!-- row -->
