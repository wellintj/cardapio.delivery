			...
			<form action="<?= base_url('admin/home/add_how_it_works') ?>" method="post" class="skill_form" enctype="multipart/form-data">
				...
				<div class="box-footer">
					...
					<?php if (isset($data['id']) && html_escape($data['id']) != 0) { ?>
						<div class="pull-left">
							<input type="hidden" name="image" value="<?= isset($data['images']) && !empty($data['images']) ? $data['images'] : ""; ?>">
									...
									<tr>
										...
										<td>
											<div class="serviceImgs">
												<img src="<?= avatar($row['thumb'], '', 'logo'); ?>" alt="how_it_works">
											</div>
										</td>
										<td>
											<a href="javascript:;" data-id="<?= html_escape($row['id']); ?>" data-status="<?= html_escape($row['status']); ?>" data-table="how_it_works" class="label <?= $row['status'] == 1 ? 'label-success' : 'label-danger' ?> change_status"> <i class="fa <?= $row['status'] == 1 ? 'fa-check' : 'fa-close' ?>"></i>&nbsp; <?= $row['status'] == 1 ? (!empty(lang('live')) ? lang('live') : "Live") : (!empty(lang('hide')) ? lang('hide') : "Hide"); ?></a>
										</td>
										<td>
											<a href="<?= base_url('admin/home/edit_how_it_works/' . html_escape($row['id'])); ?>?action=clone" class="btn btn-secondary btn-sm"><i class="fa fa-copy"></i> <?= !empty(lang('clone')) ? lang('clone') : "clone"; ?></a>
