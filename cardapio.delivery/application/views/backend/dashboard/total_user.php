            ...
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <?php if (is_access('add-user') == 1) : ?>
                        <a href="#addUserModal" data-toggle="modal" class="btn btn-secondary"><i class="fa fa-plus"></i> <?= __('add_restaurant'); ?></a>
                    <?php endif; ?>
                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="package" id="" class="form-control">
                                            ...
                                            <?php foreach ($packages as $key => $type) : ?>
                                                <option value="<?= $type['slug']; ?>" <?= isset($_GET['package']) && $_GET['package'] == $type['slug'] ? "selected" : ''; ?>>
                                                    <?= $type['package_name']; ?></option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            foreach ($all_user as $key => $row) : ?>
                                <tr>
                                    <td><?= $i; ?></td>
                                    <td><a href="<?= base_url($row['username']); ?>" target="blank" class="link_profile" data-toggle="tooltip" data-placement="top" title="Click to view profile"><?= html_escape(strtolower($row['username'])); ?></a>
                                    </td>
                                    <td><?= html_escape($row['email']); ?></td>
                                    <td> <?= $row['start_date'] != '0000-00-00 00:00:00' ? (full_time(html_escape($row['start_date']))) : (!empty(lang('not_verified')) ? lang('not_verified') : "Not Verify yet"); ?>
                                    </td>
                                    <td>
                                        ...
                                        <?php else : ?>
                                            <span class="label label-danger mr-5" data-toggle="tooltip" data-placement="top" title="<?= 'Expired on: ' . day_left(d_time(), $row['end_date'])['day_left']; ?>">
                                                <?= !empty(lang('expired')) ? lang('expired') : "Expired"; ?></span>
                                        <?php endif ?>
                                    ...
                                    <td class="rtl_flex">
                                        <?php if (is_access('change-user-operation') == 1) : ?>
                                            <?php if ($row['is_active'] == 1) { ?>
                                                <a href="<?= base_url('admin/dashboard/change_status/' . $row['id'] . "/" . $row['is_active']); ?>" class="label label-success ml-5 action_btn" data-msg="Do you want to hold this account?" data-toggle="tooltip" data-placement="top" title="Click to Deactive"><i class="fa fa-check"></i>&nbsp;
                                                    <?= !empty(lang('active')) ? lang('active') : "Active"; ?> </a>
                                            <?php } else { ?>
                                                <a href="<?= base_url('admin/dashboard/change_status/' . $row['id'] . "/" . $row['is_active']); ?>" class="label label-danger ml-5 action_btn" data-msg="<?= lang('want_to_active_this_account'); ?>" data-toggle="tooltip" data-placement="top" title="Click to Active"><i class="fa fa-ban"></i> &nbsp;
                                                    <?= !empty(lang('deactive')) ? lang('deactive') : "Dactive"; ?> </a>
                                            <?php } ?>


                                            <?php if ($row['is_verify'] == 1) : ?>
                                                <a href="javascript:;" class="label label-success ml-5" data-toggle="tooltip" data-placement="top" title="Email Verified"><i class="fa fa-check"></i>&nbsp;
                                                    <?= !empty(lang('verified')) ? lang('verified') : "Verified"; ?></a>
                                            <?php else : ?>
                                                <a href="<?= base_url('admin/dashboard/verify_by_admin/' . html_escape($row['username']. "/1") ); ?>" class="label label-danger ml-5 action_btn" data-msg="<?= lang('want_to_verify_this_account'); ?>?" data-toggle="tooltip" data-placement="top" title="Email Verify"><i class="fa fa-ban"></i>&nbsp;
                                                    <?= !empty(lang('not_verified')) ? lang('not_verified') : "Not verified"; ?></a>
                                            <?php endif ?>


                                            <?php if ($row['is_payment'] == 1) : ?>
                                                <a href="javascript:;" class="label label-success ml-5" data-toggle="tooltip" data-placement="top" title="Payment status" data-msg="<?= lang('payment_is_verified'); ?>"><i class="fa fa-check"></i>&nbsp;<?= !empty(lang('paid')) ? lang('paid') : "Paid"; ?>
                                                </a>
                                            <?php else : ?>
                                                <a href="<?= base_url('admin/dashboard/payment_by_admin/' . html_escape($row['username']. "/1") ); ?>" class="label label-danger ml-5 action_btn" data-toggle="tooltip" data-placement="top" title="Click for paid Offline Payment" data-msg="<?= lang('verified_offline_payment_msg'); ?>"><i class="fa fa-money"></i>&nbsp;
                                                    &nbsp;<?= !empty(lang('pending')) ? lang('pending') : "pending"; ?></a>
                                            <?php endif ?>
                                        <?php endif ?>
                                    ...
                                    <td>
                                        ...
                                        <?php if (is_access('change-package') == 1) : ?>
                                            <a href="#packageModal_<?= $row['id']; ?>" title="Change Package" data-toggle="modal" class="btn btn-info btn-sm btn-flat"><i class="fa fa-toggle-on"></i>
                                                <?= !empty(lang('change_package')) ? lang('change_package') : "Change package"; ?></a>
                                        <?php endif ?>
                                    </td>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><?= !empty(lang('change_packege')) ? lang('change_packege') : "Change Packages"; ?>
                    </h4>
                </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="package_id" id="" class="form-control">
                                    <?php foreach ($packages as $key => $modal_package) : ?>
                                        <option <?= $users['account_type'] == $modal_package['id'] ? "selected" : ""; ?> value="<?= $modal_package['id']; ?>"><?= $modal_package['package_name']; ?> -
                                            <?= get_currency('icon') . ' ' . $modal_package['price']; ?> /
                                            <?= $modal_package['package_type']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?php echo base_url('admin/dashboard/new_user') ?>" method="post">
                <?= csrf(); ?>
                <div class="modal-header">
                    <h4 class="modal-title"><?= lang('add_restaurant'); ?></h4>
