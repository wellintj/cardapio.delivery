...
----------------------------------------------*/
	public function insert_features($user_id)
	{
		...
		$check_feature = $this->admin_m->select_all_by_user_id_ln($user_id, 'users_active_features');

		if (sizeof($check_feature) == 0) {
		...
		} elseif (sizeof($check_feature) == sizeof($fetaures)) {
			return true;
		} elseif (sizeof($check_feature) < sizeof($fetaures)) {
	...
	public function add_order_types($user_id, $package_id = null)
	{

		$package = $this->admin_m->single_select_by_id($package_id, 'packages');

		$order_types = isset($package['order_types']) && isJson($package['order_types']) ? json_decode($package['order_types'], true) : [];
		...
		// Function to determine is_package
		$get_is_package = function ($type_id) use ($order_types) {
			if (empty($order_types)) {
				return 1;
			}
			return in_array($type_id, $order_types) ? 1 : 0;
		};
		...
		if (count($check_feature) == 0) {
			foreach ($fetaures as $key => $row) {
				$data = array(
					...
					'shop_id' => $shop_id,
					'status' => 1,
					'is_package' => $get_is_package($row['id']),
					'created_at' => d_time(),
				);
			...
			foreach ($check_feature as $feature) {
				if (isset($feature['id']) && is_numeric($feature['id'])) {
					$this->admin_m->update(
						['is_package' => $get_is_package($feature['type_id'])],
						$feature['id'],
			...
			foreach ($fetaures as $key => $row) {
				...
				if (!isset($feature_id['type_id']) || $feature_id['type_id'] != $row['id']) {
					$data = array(
						'type_id' => $row['id'],
						'user_id' => $user_id,
						'status' => 1,
					...
					$this->admin_m->insert($data, 'users_active_order_types');
				} elseif (isset($feature_id['id']) && is_numeric($feature_id['id'])) {
					$this->admin_m->update(
						['is_package' => $get_is_package($row['id'])],
						$feature_id['id'],
						'users_active_order_types'
	...
	protected function createQr($username)
	{
		...
		$qr_image = $username . '_' . random_string() . '.png';
	...
	public function pusher($id, $action_name, $order_id = [])
	{
		try {
			...
			$message = 'PusherException: ' . $e->getMessage();
			if ($this->env == true) {
				echo $message; // Show the error in development mode
			} else {
				log_message('error', $message); // Log the error in production mode
			}
		} catch (\Exception $e) {
			$message = 'Exception in Pusher: ' . $e->getMessage();
			if ($this->env = true) {
				echo $message; // Show the error in development mode
			} else {
				log_message('error', $message); // Log the error in production mode
	...
	//push notification sound
	public function msgpusher($sender_id, $receiver_id, $role, $action_name)
	{
		...
		if (isset($push->status) && $push->status == 1 && !empty($push->app_id)) :
			...
			$data = json_encode(['st' => 1, 'action_name' => $action_name, 'sender_id' => $sender_id, 'receiver_id' => $receiver_id, 'role' => $role]);
			$pusher->trigger($shop_name, 'chat', $data);
		endif;
	}
