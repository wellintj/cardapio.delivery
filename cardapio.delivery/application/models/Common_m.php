...
class Common_m extends CI_Model
{
	...
	public function get_id_by_uid($uid, $table)
	{
		$this->db->select();
		$this->db->from($table);
	...
	public function select_all_active_by_shop($id, $table)
	{
		$this->db->select();
		$this->db->from($table);
	...
	function select_with_status_ln($table)
	{
		$this->db->select();
		$this->db->from($table);
		$this->db->where('status', 1);
	...
	function get_home_packages($ref_code)
	{

		$package_type = array_merge(['free'], trial_type());

		if (!empty($ref_code)) {
			$this->db->select();
			$this->db->from('packages');
			$this->db->where('status', 1);
			$this->db->where_not_in('package_type', $package_type);
			$this->db->order_by('id', 'ASC');
		} else {
			$this->db->select();
			$this->db->from('packages');
	...
	public function single_select_by_slug_row($slug, $table)
	{
		$this->db->select();
		$this->db->from($table);
		$this->db->where('slug', $slug);
	...
	public function get_all_users($per_page, $offset, $total)
	{
		...
		$this->db->select('u.id,u.username,u.name,rl.id as shop_id,rl.thumb as logo,rl.username as shop_username,rl.name as shop_name,rl.thumb,rl.slogan,u.qr_link');
		$this->db->from('users u');
		$this->db->join('restaurant_list rl', 'rl.user_id = u.id', 'INNER');
	...
	{
		$this->db->select('u.id as user_id,rl.id as shop_id,rl.name');
	...
	public function get_single_package_specilities($id)
	{
		...
		$query = $this->db->get();
		$query = $query->result_array();
		if (isset($query[0]['is_special']) && $query[0]['is_special'] == 0) :
			foreach ($query as $key => $value) {
	...
	public function get_all_home_menu_packages($id, $limit = 0, $language = null)
	{
	...
	public function get_all_menu_packages($id, $limit = 0, $language = null)
	{
		$language = !empty($language) ? $language : $this->site_lang;
		$this->db->select('ip.*');
		$this->db->from('item_packages ip');
	...
	{
		$this->db->select('ip.*,ip.tax_fee as package_tax_fee,ip.tax_status as ip.package_tax_status');
		$this->db->from('item_packages ip');
	...
	public function get_all_items_by_user($id, $limit = 0, $product_limit = 8, $isHome = 0)
	{
	...
	public function get_item_by_cat_id_ln($user_id, $cat_id, $limit = 0, $per_page = 0, $offset = 0, $total = 0, $language = null)
	{
		$language = !empty($language) ? $language : $this->site_lang;
		...
		if (isset($_GET['item']) && !empty($_GET['item'])) {
			$this->db->like('TRIM(i.title)', trim($_GET['item']));
		}
	...
	public function get_specilities($id, $limit = 0, $language = null)
	{
		$language = !empty($language) ? $language : $this->site_lang;
		$this->db->select('mt.*');
	...
	public function get_home_specilities($id, $limit = 0, $language = null)
	{
		$language = !empty($language) ? $language : $this->site_lang;
		$this->db->select('mt.*');
		$this->db->from('item_packages mt');
	...
	public function track_all_orders($phone, $shop_id, $orderId, $per_page, $offset, $total)
	{
		$this->db->select('ol.*');
		$this->db->from('order_user_list ol');
		$this->db->where('ol.shop_id', $shop_id);
	...
	public function check_active_features($id, $slug)
	{
		...
		$this->db->where('uf.language', 'english');
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
			return ['check' => 1];
		} else {
	...
	public function get_order_details($uid)
	{
		$this->db->select();
		$this->db->from('order_user_list');
		$this->db->where('uid', $uid);
		...
		if (!empty($query)) {
			$query['item_list'] = $this->get_order_item($query['id'], $query['shop_id']);
		};
	...
	{
		$this->db->select('oi.*');
		$this->db->select('i.id as items_id,i.title as name,i.thumb as item_thumb,i.cat_id,i.allergen_id,i.veg_type,i.price,a.name as allergen,i.tax_fee,i.tax_status,i.img_type,i.img_url');
		$this->db->select('ip.package_name,ip.thumb as package_thumb,ip.final_price');
		$this->db->from('order_item_list oi');
		$this->db->join('items i', 'i.id = oi.item_id', 'LEFT');
		$this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
	...
	{
		$this->db->select('r.id as shop_id,r.username as shop_username,r.name as shop_name,r.thumb,r.slogan,u.qr_link');
		$this->db->from('restaurant_list r');
		$this->db->join('users u', 'u.id = r.user_id', 'RIGHT');
	...
	public function check_latest_order($shop_id, $order_type, $email, $phone, $customer_id)
	{
		...
		if (isset($customer_id) && $customer_id != 0 && !empty($customer_id)) :
...
{
	$sess = ['is_merge', 'is_table', 'table_no', 'payment', 'qr_order', 'order_info', 'is_pos', 'is_order_edit', 'customer_info'];
...
public function get_cat_info_by_id_ln($user_id, $cat_id, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
	$this->db->select('mi.*');
	$this->db->from('menu_type mi');
	...
	$this->db->where('mi.user_id', $user_id);
	$this->db->where('mi.status', 1);
	$this->db->where('mi.language', $language);
	$this->db->order_by('mi.id', 'ASC');
	$query = $this->db->get();
	$query = $query->row_array();
	return $query;
...
public function get_my_page_by_slug($slug, $shop_id, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
	$this->db->select('v.*');
	$this->db->from('vendor_page_list v');
	$this->db->where('v.slug', $slug);
	$this->db->where('v.shop_id', $shop_id);
	$this->db->where('v.status', 1);
	$this->db->where('v.language', $language);
	$query = $this->db->get();
	if ($query->num_rows() > 0)
		$query =  $query->row();
	else {
		...
		$this->db->where('v.shop_id', $shop_id);
		$this->db->where('v.status', 1);
		$this->db->where('v.language', $this->language);
		$query = $this->db->get();
		$query = $query->row();
	}
...
public function get_my_page_by($shop_id, $type = 'menubar', $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
	$this->db->select('v.*');
	$this->db->from('vendor_page_list v');
...
public function check_order_status($order_id, $type = '')
{
	$this->db->select('o.dboy_status,dboy_id,uid');
	$this->db->from('order_user_list o');
	$this->db->where('o.uid', $order_id);
	$query = $this->db->get();
	if ($query->num_rows() > 0) {
		return $query->row();
	} else {
...
public function get_orders_for_staff_details()
{
	$this->db->select('u.staff_id,staff_action,action_by,id,shop_id,created_at');
	$this->db->from('order_user_list u');
	$this->db->where('u.staff_id IS NOT NULL', NULL, FALSE);
	$this->db->where('u.staff_action IS NOT NULL', NULL, FALSE);
	$this->db->where('u.staff_id !=', 0);
	$query = $this->db->get();
	$query = $query->result();
	return $query;
}

public function check_country_by_dial_code($dial_code)
{
	$this->db->select('*');
	$this->db->from('country ');
	$this->db->where('dial_code', $dial_code);
	$query = $this->db->get();
	if ($query->num_rows() > 0) {
		return $query->row();
	} else {
...
public function select_by_shop($table, $id = 0)
{
	...
	// sub category
public function sub_category_details($user_id, $sub_cat_id, $is_md5 = false, $language = null)
{
	$id = md5('1');
	$language = !empty($language) ? $language : $this->site_lang;
	$this->db->select('s.*');
	$this->db->from('sub_category_list s');
	if ($is_md5 == false) {
		$this->db->where('s.id', $sub_cat_id);
	} else {
		$this->db->where('md5(s.id)', $sub_cat_id);
	}
...
public function get_item_by_sub_cat_id($user_id = 0, $cat_id = 0, $sub_cat_id = 0, $limit = 0, $per_page = 0, $offset = 0, $total = 0, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;

	$this->db->select('i.*,i.id as item_id,a.name as allergen,c.id,c.status,s.sub_category_name');
	$this->db->from('items i');
	$this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
	$this->db->join('menu_type c', 'c.category_id = i.cat_id', 'LEFT');
	$this->db->join('sub_category_list s', 's.id = i.sub_category_id', 'LEFT');
	$this->db->where('i.user_id', $user_id);
	if ($cat_id != '0') :
		$this->db->where('md5(i.cat_id)', $cat_id);
	endif;
...
public function get_my_offers($slug, $shop_id)
{
	$today = today();
...
public function get_featured_items($shop_id = 0, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
...
public function get_popular_items($shop_id = null, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
...
public function get_categories($id = null, $language = null)
{
	$language = !empty($language) ? $language : $this->site_lang;
