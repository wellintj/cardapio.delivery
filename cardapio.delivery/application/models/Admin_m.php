...
class Admin_m extends CI_Model
{
    ...
    {
        $result = $this->security_m->update($data, $id, $table);
        if ($result === false) {
            // return ["error" => "Too many suspicious attempts. Please try again later."];
            return true;
        } else {
            return $result;
        }
    ...
    {
        $result = $this->security_m->update_in($data, $ids, $table);
        if ($result === false) {
            // return ["error" => "Too many suspicious attempts. Please try again later."];
            return true;
        } else {
    ...
    {
        $result = $this->security_m->insert($data, $table);
        if ($result === false) {
            // return ["error" => "Too many suspicious attempts. Please try again later."];
            return true;
        } else {
    ...
    public function __update($data, $id, $table)
    {
        $this->db->where('id', $id);
    ...
    {
        $result = $this->system_m->clean($data);
        if ($result === false) {
            // return ["error" => "Too many suspicious attempts. Please try again later."];
            return true;
        } else {
    ...
    {
        $settings = settings();
        return isset($settings['is_city_delivery']) && $settings['is_city_delivery'] == 1 ? 1 : 0;
    }
    ...
    public function update_all($data, $table, $type = 'id')
    {
        // Insert order items
        $insert = $this->db->update_batch($table, $data, $type);
    ...
    public function delivery_login_with_email($email, $password)
    {

        $this->db->select('st.*');
        $this->db->from('staff_list st');
        $this->db->where("st.email", $email);
        $this->db->where("st.role", 'delivery');
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
            $result = $query->result();
    ...
    public function delivery_login_with_phone($phone_with_code, $phone_without_code, $password)
    {
        $this->db->select('st.*');
        $this->db->from('staff_list st');
        $this->db->where("(st.phone = '$phone_with_code' OR st.phone = '$phone_without_code')");
        $this->db->where("st.role", 'delivery');
        $this->db->limit(1);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
    ...
    public function check_customer_login_status($phone_with_code, $phone_without_code, $password)
    {
        $this->db->select('st.*');
    ...
    public function in_update($data, $ids, $table)
    {
        if (!empty($ids) && is_array($ids)) :
            $this->db->where_in('id', $ids);
            $this->db->update($table, $data);
    ...
    public function check_valid_customer_email($email)
    {
        $this->db->select('u.*');
        $this->db->from('customer_list u');
        $this->db->where('u.email', $email);
    ...
    public function single_select_by_package_id($id, $table)
    {
        $this->db->select();
        $this->db->from($table);
    ...
    public function get_shop_info_md5($shop_id)
    {
    ...
    public function get_order_types()
    {
        ...
        $this->db->where('slug !=', 'pay-in-cash');
        $this->db->where('status', 1);
        $query = $this->db->get();
        $query = $query->result_array();
        return $query;
    ...
    public function single_select_by_section_name($section_name, $language = '')
    {
        $language = !empty($language) ? $language : $this->site_lang;

        $this->db->select('*');
        $this->db->from('section_banners');
        $this->db->where('section_name', $section_name);
        if ($this->db->field_exists('language', 'section_banners')) {
    ...
    public function get_single_section($section_name, $language = '')
    {
        $language = !empty($language) ? $language : $this->site_lang;
    ...
    function delete_all_by_vendor_id($id, $table)
    {
        $this->db->delete($table, array('shop_id' => $id));
        return 1;
    }
    ...
    public function select_ln($table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('language', $this->site_lang);
    ...
    public function get_invoice_details($user_id, $id)
    {
        if ($id == 0) :
            $this->db->select('p.*,p.slug as package_slug');
            $this->db->from('packages p');
            ...
            $this->db->select('p.*,p.slug as package_slug,p.price as package_price');
            $this->db->from('packages p');
            $this->db->where('p.id', $id);
            $query = $this->db->get();
            $query = $query->row();
    ...
    public function select_all_by_vendor($table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('shop_id', $_ENV['ID']);
        $this->db->order_by('id', 'DESC');
        $query = $this->db->get();
        $query = $query->result();
        return $query;
    ...
    public function select_all_by_shop_status($id, $table)
    {
    ...
    public function select_all_by_user_id_ln($id, $table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('user_id', $id);
    ...
    public function select_all_by_user_ln($table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('user_id', auth('id'));
    ...
    public function select_all_by_vendor_ln($shop_id, $table)
    {

        $ID = !empty($shop_id) ? $shop_id : $_ENV['ID'];
        $this->db->select();
        $this->db->from($table);
        $this->db->where('shop_id', $ID);
        $this->db->where('language', $this->site_lang);
    ...
    {
        $this->db->select('uf.*,f.features as name,f.slug,is_header');
        $this->db->from('users_active_features uf');
        $this->db->join('features f', 'f.id=uf.feature_id', 'LEFT');
        $this->db->where('f.status', 1);
        $this->db->where('uf.user_id', auth('id'));
        $this->db->where('uf.language', $this->site_lang);
        $this->db->order_by('f.is_header', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) :
            $query = $query->result_array();
        else :
            ...
            $this->db->where('f.status', 1);
            $this->db->where('uf.user_id', auth('id'));
            $this->db->where('uf.language', $this->language);
            $this->db->order_by('f.is_header', 'DESC');
            $query = $this->db->get();
            $query = $query->result_array();
        endif;
    ...
    public function update_feature($data, $id, $table, $language)
    {
    ...
    {
        $this->db->select('i.*,a.name as allergen,i.thumb as item_thumb,m.id as category_id');
        $this->db->from('items i');
        $this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
    ...
    public function get_all_items_by_user($id, $limit = 0, $item_limit = 8, $isHome = 0)
    {
        $this->db->select('mt.*');
        $this->db->from('menu_type mt');
        $this->db->where('mt.user_id', $id);
        if ($limit != 0) {
            $this->db->limit($limit);
        }
    ...
    public function get_all_packages($id)
    {
        ...
        $this->db->join('item_packages_list il', 'il.id = ip.package_id AND ip.language = ' . $this->db->escape($this->site_lang));
        $this->db->where('ip.user_id', auth('id'));
        $this->db->where('ip.shop_id', $_ENV['ID']);
        $this->db->where('ip.is_special', 0);
        if ($id != 0) {
            $this->db->where('id', $id);
        }
        $this->db->order_by('id', 'DESC');
        ...
        foreach ($query as $key => $value) {
            ...
            $this->db->from('items i');
            $this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
            $this->db->where_in('i.id', __jsonDecode($value['item_id']));
            $this->db->where('i.status', 1);
            $this->db->order_by('i.orders', 'ASC');
            $query2 = $this->db->get();
            $query2 = $query2->result_array();
    ...
    public function get_my_order_list_by_id($id, $per_page, $offset, $total)
    {
        ...
        if (isset($_GET)) {
            ...
            if (!empty($get['order_type'])) {
                if ($get['order_type'] == 'pos') {
                    $this->db->where('ol.is_pos', 1);
                } else {
            ...
            if (!empty($get['payment_by'])) {
                $this->db->where('ol.payment_by', $get['payment_by']);
            }

            if (isset($get['status']) && $get['status'] != 'all') {
                if ($get['status'] == 'draft') {
                    $this->db->where('ol.is_draft', 1);
                    $this->db->where('ol.status', 0);
                } else {

                    $this->db->where('ol.is_draft', 0);
                }

                if ($get['status'] == 'paid') {
                    $this->db->where('ol.is_payment', 1);
                }

                $this->db->where('ol.status', Order::status($get['status']));
            } else {
                $this->db->where('ol.is_draft', 0);
            }


            $daterange = __date_range($this->input->get('daterange', TRUE));

            if (!empty($daterange)) {
                $newDate1 = $daterange->newDate1;
                $newDate2 = $daterange->newDate2;
    ...
    public function get_my_reservation_list_by_id($id, $per_page = 0, $offset = 0, $total = 0)
    {
        $this->db->select('ol.*');
        $this->db->from('order_user_list ol');
        ...
        if (isset($_GET)) {
            ...
            if (!empty($daterange)) {
                ...
                $newDate2 = $daterange->newDate2;
                $this->db->where("(
                    (DATE(ol.reservation_date) BETWEEN '$newDate1' AND '$newDate2') OR 
                    (DATE(ol.created_at) BETWEEN '$newDate1' AND '$newDate2')
                )")->where('ol.order_type', 3)->where('ol.shop_id', $id);
            }
        }
    ...
    public function get_my_todays_dine($id)
    {
        ...
        foreach ($query as $key => $value) {
            ...
            foreach ($query2 as $key2 => $value2) {
                ...
                $this->db->from('items i');
                $this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
                $this->db->where_in('i.id', __jsonDecode($value2['item_id']));
                $this->db->where('i.status', 1);
                $this->db->order_by('i.orders', 'ASC');
                $query3 = $this->db->get();
    ...
    public function get_item_list_by_order_id($id, $shop_id)
    {
        ...
        foreach ($query as $key => $value) {
            ...
            $this->db->select('ip.package_name,ip.thumb as package_thumb,ip.final_price,ip.tax_status as package_tax_status,ip.tax_fee as package_tax_fee');
            $this->db->from('order_item_list oi');
    ...
    public function get_item_sizes($item_id, $shop_id)
    {
        $this->db->select('i.id,is_size,price,shop_id');
        $this->db->from('items i');
        ...
        $query = $this->db->get();
        $query = $query->row();
        if (isset($query->is_size) && $query->is_size == 1) :
            return isJson($query->price) ? __jsonDecode($query->price) : '';
        else :
            return '';
        endif;
    }
    ...
    public function get_package_items()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->where("i.is_size", 0);
            $this->db->where('i.is_pos_only !=', 1);
            $this->db->where('i.language', $this->language);
            $this->db->order_by('i.orders', 'ASC');
            $query = $this->db->get();
            $query = $query->result_array();
        }
    ...
    public function get_time_config($id)
    {
        $shop_id = shop_id();
        $this->db->select('r.time_config');
        $this->db->from('reservation_date r');
        $this->db->where('r.id', $id);
        $this->db->where('r.time_config is NOT NULL', NULL, FALSE);
        $query = $this->db->get();
        if ($query->num_rows() > 0) :
            $data =  $query->row();
            return isJson($data->time_config) ? __jsonDecode($data->time_config) : $data->time_config;
        else :
            return '';
        endif;
    }
    ...
    public function get_my_today_order_list_by_kds_id($id)
    {
        ...
        $this->db->where("(DATE_FORMAT(ol.accept_time,'%Y-%m-%d') = '{$today}' OR DATE_FORMAT(ol.created_at,'%Y-%m-%d') = '{$today}')", NULL, FALSE);
        $this->db->where('ol.order_type !=', 3);
        ...
        if (isset($_GET['q']) && !empty($_GET['q'])) {
            switch ($_GET['q']) {
                ...
                case 'new-order':
                    ...
                    $this->db->where('is_preparing', 0);
                    break;
                case 'preparing':
                    $this->db->where('ol.status', 1);
                    $this->db->where('is_preparing', 1);
                    break;
                case 'ready':
                    $this->db->where('ol.status', 1);
                    $this->db->where('is_preparing', 2);
                    break;
                case 'served':
        ...
        if (isset($_GET['orderid']) && !empty($_GET['orderid'])) {
            $this->db->where('ol.uid', $_GET['orderid']);
        }

        $this->db->order_by('is_preparing asc, status asc, created_at desc');
        $query = $this->db->get();
        $query = $query->result_array();
        foreach ($query as $key => $value) {
            $this->db->select('SUM(oi.sub_total) as total_price');
        ...
        foreach ($query as $key => $value) {
            $this->db->select('oi.*,oi.id as order_item_id');
    ...
    public function get_my_all_staf()
    {
        ...
        foreach ($query as $key => $value) {
            ...
            $this->db->where_in('p.id', __jsonDecode($value['permission']));
        ...
        foreach ($query as $key2 => $value2) {
            $query[$key2]['actions'] = $this->get_action_type_counts_for_staff($value2['id']);
        }
    ...
    {
        $this->db->select('ol.*,c.city_name,c.state');
        $this->db->from('staff_list ol');
        $this->db->join('restaurant_city_list c', 'c.id = ol.city_id', "LEFT");
        $this->db->where('ol.user_id', auth('id'));
    ...
    public function check_permission($type)
    {
        ...
        if (auth('is_staff') == TRUE) :
            ...
            $query = $this->db->get();
            $query = $query->row_array();
            if (in_array($this->get_id_by_slug($type), __jsonDecode($query['permission'], true))) {
                return 1;
            } else {
                return 0;
            }
    ...
    public function check_customer_login($phone_with_code, $phone_without_code, $password)
    {
        $this->db->select('u.*');
        $this->db->from('customer_list u');
        $this->db->where("(u.phone = '$phone_with_code' OR u.phone = '$phone_without_code' OR u.email = '$phone_without_code')");
    ...
    public function check_customer_email_login($email, $password)
    {
        $this->db->select('u.*');
        $this->db->from('customer_list u');
        $this->db->where("u.email", $email);
    ...
    public function get_my_today_reservation_list_by_id($id)
    {
        ...
        // Use where instead of where_in for a single value
        ...
        if (isset($_GET)) {
            ...
            if (!empty($daterange)) {
                $newDate1 = $daterange->newDate1;
                $newDate2 = $daterange->newDate2;
                $this->db->where("DATE_FORMAT(ol.reservation_date,'%Y-%m-%d') >=", $newDate1);
                $this->db->where("DATE_FORMAT(ol.reservation_date,'%Y-%m-%d') <=", $newDate2);
            }
        }

        $this->db->where('ol.shop_id', $id);
        $this->db->where("(DATE_FORMAT(ol.created_at,'%Y-%m-%d') = '$today' OR DATE_FORMAT(ol.reservation_date,'%Y-%m-%d') = '$today')");
        $this->db->order_by('status asc, created_at desc, reservation_date');
    ...
    public function get_customer_order_list($id, $per_page, $offset, $total = 0)
    {
    ...
    public function get_all_delivery_order_list($user_id, $status, $is_picked)
    {
        ...
        $this->db->select('or.*, rl.name as shop_name, rl.id as restaurant_id, rl.city_id as shop_city_id');
        $this->db->select('sl.name as staff_name, sl.phone as staff_phone, sl.country_id, sl.city_id as staff_city_id');
        $this->db->from('order_user_list or');
        $this->db->join('restaurant_list rl', 'rl.id = or.shop_id', 'left');
        $this->db->join('staff_list sl', 'sl.user_id = rl.user_id', 'left');

        // Apply city-specific conditions
        if ($isCity == 1) {
            $this->db->where('sl.city_id = rl.city_id');
        } else {
            $this->db->where('rl.user_id', $user_id);
        }

        $this->db->where('or.status', 2);
        $this->db->where('or.order_type', 1);
        $this->db->where('or.dboy_status', 0);
        $this->db->where("DATE_FORMAT(or.completed_time, '%Y-%m-%d') BETWEEN '$last7' AND '$today'");
        $this->db->group_by('or.uid');
        $this->db->order_by('or.id', 'DESC');

        $subquery = '(SELECT SUM(oi.qty) FROM order_item_list oi WHERE oi.order_id = or.id) AS total_item';
    ...
    public function top_10_popular_customers__()
    {
        $id = restaurant()->id;
        ...
        array_multisort(array_column($query, 'total_amount'), SORT_DESC, $query);
    ...
    public function top_10_popular_customers()
    {
        ...
        $this->db->select('o.customer_id, SUM(oi.item_price) AS total_amount, SUM(oi.qty) AS total_item,c.customer_name');
        $this->db->from('order_user_list o');
        $this->db->join('order_item_list oi', 'o.id = oi.order_id', 'INNER');
        $this->db->join('customer_list c', 'c.id = o.customer_id', 'LEFT');
        $this->db->where('o.shop_id', $_ENV['ID']);
    ...
    public function top_10_popular_item()
    {
        ...
        if (!empty($order_type) && $order_type != 'all') {
            $this->db->where('ou.order_type', $order_type);
        }

        if (!empty($status) && $status != 'all') {
            $this->db->where('ou.status', $status);
        }

        $daterange = __date_range($this->input->get('daterange', TRUE));
        if (!empty($daterange)) {
            $this->db->where("DATE_FORMAT(or.created_at,'%Y-%m-%d') >=", $daterange->newDate1);
            $this->db->where("DATE_FORMAT(or.created_at,'%Y-%m-%d') <=", $daterange->newDate2);
        }
    ...
    public function top_10_sell_qty_item()
    {
        ...
        foreach ($query as $key => $value) {
            ...
            if (isset($_GET)) {
                ...
                $daterange = __date_range($this->input->get('daterange', TRUE));
                if (!empty($daterange)) {
                    $this->db->where("DATE_FORMAT(oi.created_at,'%Y-%m-%d') >=", $daterange->newDate1);
                    $this->db->where("DATE_FORMAT(oi.created_at,'%Y-%m-%d') <=", $daterange->newDate2);
                }
            } else {
                $this->db->where('ol.status', 2);
            }
    ...
    public function select_by_shop_id_status($id, $table)
    {
        $id = empty($id) ? restaurant()->id : $id;
        $this->db->select();
        $this->db->from($table);
        $this->db->where('shop_id', $id);
        $this->db->where('status', 1);
    ...
    public function select_by_shop($table, $id = 0)
    {
        $id = empty($id) ? restaurant()->id : $id;
    ...
    protected function get_total_price($ids)
    {
        $this->db->select('SUM(ul.total) as total_price');
        $this->db->from('order_user_list ul');
        $this->db->where_in("ul.id", __jsonDecode($ids));
        $query = $this->db->get();
        $query = $query->row_array();
    ...
    protected function get_total_items($ids)
    {
        $this->db->select('SUM(oi.qty) as total_item');
        $this->db->from('order_item_list oi');
        $this->db->where_in("oi.order_id", __jsonDecode($ids));
        $query = $this->db->get();
        $query = $query->row_array();
    ...
    public function call_waiter_active_notify($shop_id, $table_id)
    {
        $today = today();
        $this->db->select('or.*');
        $this->db->from('call_waiter_list or');
    ...
    public function update_with_uid($data, $id, $table)
    {
        $this->db->where('uid', $id);
        $this->db->update($table, $data);
        return $id;
    ...
    public function get_all_items_by_user_ln($id, $limit = 0, $item_limit = 8, $isHome = 0, $language = '')
    {
        $language = !empty($language) ? $language : $this->site_lang;
        $this->db->select('mt.*');
        $this->db->from('menu_type mt');
        ...
        foreach ($query as $key => $value) {
            $this->db->select('s.*');
            $this->db->from('sub_category_list s');
            $this->db->where('s.category_id', $value['category_id']);
            $this->db->where('s.user_id', $value['user_id']);
            $this->db->where('s.status', 1);
            $this->db->where('s.is_pos_only', 0);
            $this->db->where('s.language', $language);
            $this->db->group_by('s.id');
            $this->db->order_by('s.orders', 'ASC');
            $query2 = $this->db->get();
            $query2 = $query2->result();
            $query[$key]['sub_categories'] = $query2;
        }
    ...
    public function get_user_payment_history($id = 0, $row = false)
    {
        if ($id == 0) :
            ...
            $this->db->from('payment_info o');
            $this->db->join('users u', 'u.id=o.user_id', "LEFT");
            $this->db->join('packages p', 'p.id=o.account_type', "RIGHT");
    ...
    public function get_shop_reviews()
    {
        ...
        if (isset($_GET['status']) && !empty($_GET['status'])) {
            $this->db->where('or.is_rating_approved', status($_GET['status']));
        } else {
            $this->db->where_in('or.is_rating_approved', [0]);
        }
    ...
    public function get_my_shop_reviews()
    {
        $this->db->select('or.id as order_id,or.uid,or.customer_id,or.customer_rating,or.customer_review,or.rating_time,st.id as customer_id,st.customer_name as customer_name,l.username,l.name,or.is_rating_approved');
        $this->db->from('order_user_list or');
        $this->db->join('customer_list st', 'st.id = or.customer_id', 'LEFT');
        ...
        $this->db->where('or.customer_rating!=', 0);
        $this->db->where('or.shop_id', $_ENV['ID']);
        $this->db->where_in('or.is_rating_approved', [1]);
        $this->db->where('or.customer_rating IS NOT NULL', NULL, FALSE);
        $query = $this->db->get();
        $query = $query->result_array();
        return $query;
    ...
    public function get_staff_orders_activities($staff_id, $type = 'total')
    {
        $today = today();
        $month = make_date(1, 'month');
        $this->db->select('o.*,s.id as staff_id,s.name as staff_name');
        $this->db->from('staff_order_activity_logs o');
        $this->db->join('staff_list s', 's.id = o.staff_id', 'LEFT');
        $this->db->where('o.staff_id', $staff_id);
        if ($type == 'today') {
            $this->db->where("DATE_FORMAT(o.created_at, '%Y-%m-%d') = '$today'");
        }
        if ($type == 'lastMonth') {
            $this->db->where("DATE_FORMAT(o.created_at,'%Y-%m-%d') <=", $today);
            $this->db->where("DATE_FORMAT(o.created_at,'%Y-%m-%d') >=", $month);
        }

        if ($type == 'month') :
            $this->db->where("DATE_FORMAT(o.created_at,'%m')", date("m"));
        endif;
        $this->db->group_by('order_id');
        $query = $this->db->get();
        $query = $query->result();
        return ['result' => $query, 'total' => sizeof($query)];
    }
    ...
    public function get_extras_name_by_id($ids, $item_id)
    {
        $this->db->select('ie.*');
        $this->db->from('item_extras ie');
        $this->db->where_in('ie.id', __jsonDecode($ids));
        $this->db->where('ie.item_id', $item_id);
        $query = $this->db->get();
        $query = $query->result();
        return $query;
    ...
    public function get_staff_activities($uid, $role = 'staff')
    {
        $this->db->select('u.*, s.name as staff_name,ul.order_type,ul.uid');

        $this->db->select("
        IF(COUNT(DISTINCT u.action_type) > 1,
        GROUP_CONCAT(DISTINCT u.action_type ORDER BY u.action_type),
        MAX(u.action_type)) as action_types", false);

        $this->db->from('staff_order_activity_logs u');
        $this->db->join('staff_list s', 's.id = u.staff_id', 'LEFT');
        $this->db->join('order_user_list ul', 'ul.id = u.order_id', 'LEFT');
        $this->db->where('s.role', $role);


        $daterange = __date_range($this->input->get('daterange', TRUE));
        if (!empty($daterange)) {
            $sql = "(DATE_FORMAT(u.created_at,'%Y-%m-%d') BETWEEN '{$daterange->newDate1}' AND '{$daterange->newDate2}')";
            $this->db->where($sql);
        }

        $this->db->where('s.user_id', auth('id'));
        $this->db->where('s.uid', $uid);
        $this->db->group_by('u.order_id');
    ...
    // dispatch
    public function get_my_staff_info($id = '')
    {
        $staff_id = !empty($id) ? $id : auth('staff_id');
        $this->db->select('s.*');
        $this->db->from('staff_list s');
        $this->db->where('s.id', $staff_id);
        $query = $this->db->get();
        $query = $query->result();

        foreach ($query as $key => $value) {
            $staff_actions = (object) ['create_order', 'accept', 'complete', 'reject'];
            foreach ($staff_actions as $action) {
                $this->db->select('u.*');
                $this->db->from('order_user_list u');
                $this->db->where('u.staff_id', $value->id);
                $this->db->where('u.staff_action', $action);
                $query2 = $this->db->get();
                $query[$key]->actions[$action] = $query2->num_rows();
            }
        }
        return (object) $query[0]->actions ?? [];
    }


    public function get_action_type_counts_for_staff($staff_id, $role = 'staff')
    {
        $this->db->select('u.action_type, COUNT(u.action_type) as total_count');
        $this->db->from('staff_order_activity_logs u');
        $this->db->join('staff_list s', 's.id = u.staff_id', 'LEFT');
        $this->db->where('s.role', $role);
        $this->db->where('s.id', $staff_id);
        ...
        $this->db->group_by('u.action_type');
    ...
    public function get_my_reject_reason($ids)
    {

        $this->db->select('v.*');
        $this->db->from('vendor_question_list v');
    ...
    public function get_tutorial_data()
    {
        if ($this->db->table_exists('admin_tutorial_list')) :
            $this->db->select('t.uid,page_title');
            $this->db->from('admin_tutorial_list t');
    ...
    public function get_vendor_reason_list($shop_id, $type)
    {

        $ID = !empty($shop_id) ? $shop_id : $_ENV['ID'];
        $this->db->select();
        $this->db->from('vendor_question_list');
        $this->db->where('shop_id', $ID);
        $this->db->where('access_for', $type);
        $this->db->where('language', $this->site_lang);
        $this->db->order_by('id', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) :
    ...
    public function get_manage_feature_by_language($language)
    {
        $this->db->select('uf.*,f.features as name,f.slug');
        $this->db->from('users_active_features uf');
        $this->db->join('features f', 'f.id=uf.feature_id', 'LEFT');
    ...
    public function get_table_details($table_id)
    {
        $this->db->select('tl.*,ta.area_name,ta.id as table_area_id');
        $this->db->from('table_list tl');
        $this->db->join('table_areas ta', 'ta.id=tl.area_id', 'LEFT');
    ...
    public function check_by_keys($key)
    {
        $this->db->select();
        $this->db->from('admin_config a');
        $this->db->where('a.key', $key);
        $query = $this->db->get();
        if ($query->num_rows() > 0) :
            $data = 1;
        else :
    ...
    public function update_by_keys($key = '', $data = [])
    {
        $this->db->where('key', $key);
        $this->db->update('admin_config', xs_clean($data));
        return $key;
    }

    public function check_existing_pending_payment_by_subscriber($subscriber_id, $pacakge_id)
    {
        $this->db->select('vf.*');
        $this->db->from('payment_info vf');
        $this->db->where('vf.account_type', $pacakge_id);
        $this->db->where('vf.user_id', $subscriber_id);
        $this->db->where('vf.is_payment', 0);
        $this->db->order_by('vf.created_at', "DESC");
        $query = $this->db->get();
        return $query->row();
    }

    public function get_payment_income($type = 'month')
    {
        $thisMonth = year_month(d_time());
        $this->db->select('SUM(vf.price) as total_price, 
        SUM(vf.package_price) as total_package_price');
        $this->db->from('payment_info vf');
        if ($type == 'month') :
    ...
    public function is_unique_uid($uid, $table)
    {
        $counter = 1;

        $original_uid = str_replace('-', '', $uid);

        while ($this->is_uid_exists($uid, $table)) {
            $uid = $original_uid . $counter;
            $counter++;
        }
        return $uid;
    }
    ...
    public function is_uid_exists($uid, $table)
    {
        $this->db->where('uid', $uid);
    ...
    public function get_order_item_id($itemId)
    {
        $this->db->select();
        $this->db->from('order_item_list');
        $this->db->where('md5(id)', $itemId);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->row()->id;
        } else {
    ...
    public function get_order_details($id, $shop_id, $status, $encrypt = false)
    {
        $this->db->select("oi.item_id");
        $this->db->from('order_item_list oi');
        $this->db->where('oi.order_id', $id);
        $this->db->where_in('oi.status', [0, 1]);
        if ($encrypt == true) {
            $this->db->where('md5(oi.shop_id)', $shop_id);
        } else {
            $this->db->where('oi.shop_id', $shop_id);
        }
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            ...
            foreach ($ids as $key => $value) :
                $this->update_item_ids($status, $value->item_id, $id, $shop_id, $encrypt);
            endforeach;
        } else {
            return [];
        }
    ...
    public function update_item_ids($status, $ids, $order_id, $shop_id, $is_encrypt = true)
    {
        ...
        $this->db->where('item_id', $ids);
        $this->db->update('order_item_list', ['status' => $status]);
        return 1;
    }


    public function get_todays_orders($shop_id)
    {
        $today = date('Y-m-d');
        $this->db->select('o.id as order_id,mt.id as category_id,mt.name as category_name');
        $this->db->from('order_user_list o');
        $this->db->join('order_item_list or', 'o.id = or.order_id', 'left');
        $this->db->join('items i', 'i.id = or.item_id', 'left');
        $this->db->join('menu_type mt', 'mt.id = i.cat_id', 'left');
        $this->db->where("o.shop_id", $shop_id);
        $this->db->where("(DATE(o.accept_time) = '$today' OR DATE(o.completed_time) = '$today' OR DATE(o.created_at) = '$today')");
        $this->db->group_by("mt.id");
        $query = $this->db->get();
        $query =  $query->result();
        foreach ($query as  $key => $value) :
            $this->db->select('order_user_list.id, order_item_list.item_id, order_item_list.item_price, COUNT(order_item_list.item_id) AS item_count,i.title as item_name,mt.name as category_name,i.cat_id as category_id');
            $this->db->from('order_user_list');
            $this->db->join('order_item_list', 'order_user_list.id = order_item_list.order_id', 'left');
            $this->db->join('items i', 'i.id = order_item_list.item_id', 'left');
            $this->db->join('menu_type mt', 'mt.id = i.cat_id', 'left');
            $this->db->where("(DATE(order_user_list.accept_time) = '$today' OR DATE(order_user_list.completed_time) = '$today' OR DATE(order_user_list.created_at) = '$today')");
            $this->db->where("mt.id", $value->category_id);
            $this->db->where("order_user_list.shop_id", $shop_id);
            $this->db->group_by('order_item_list.item_id');
            $query2 = $this->db->get();
            $query2 =  $query2->result();
            $query[$key]->items = $query2;
        endforeach;
    ...
    public function get_todays_kds_order_statistics($id)
    {
        $today = today();
        $this->db->select('ol.*');
        $this->db->from('order_user_list ol');
        $this->db->where('ol.shop_id', $id);
        $this->db->where("DATE_FORMAT(ol.accept_time,'%Y-%m-%d')", $today);
        $this->db->or_where("DATE_FORMAT(ol.created_at,'%Y-%m-%d')", $today);
        $this->db->where('ol.order_type !=', 3);
        $this->db->order_by('is_preparing asc, status asc, created_at desc');
        $query = $this->db->get();
        $query = $query->result_array();
    ...
    public function get_nearby_shops($target_lat, $target_lon, $max_distance = 15, $unit = 'km')
    {
        // Use 3959 for miles and 6371 for kilometers in the Haversine formula
        $earth_radius = ($unit == 'km') ? 6371 : 3959;

        $query = $this->db->select("*, ($earth_radius * acos(cos(radians($target_lat)) * cos(radians(latitude)) * cos(radians(longitude) - radians($target_lon)) + sin(radians($target_lat)) * sin(radians(latitude)))) AS distance")
            ->from('shop_location_list')
    ...
    public function get_call_waiter_list()
    {
        $today = today();
        $shop_id = shop_id();
        $this->db->select('c.*,t.name as table_name');
        $this->db->from('call_waiter_list c');
        $this->db->join('table_list t', 'c.table_no = t.id', 'LEFT');
        $this->db->where('c.shop_id', $shop_id);
        $this->db->where("DATE_FORMAT(c.created_at,'%Y-%m-%d')", $today);
        $this->db->order_by("c.created_at DESC, status ASC");
        $query = $this->db->get();
        $query = $query->result();
        return $query;
    }
    ...
    {
        $this->db->select('c.*');
        $this->db->from('vendor_menu_list c');
        $this->db->where('c.dropdown_id', $id);
        $query = $this->db->get();
        $query = $query->result_array();
        return $query;
    }
    ...
    public function get_vendor_menu($shop_id, $is_quick = 'all')
    {
        $this->db->select('c.*');
        $this->db->from('vendor_menu_list c');
        $this->db->where('c.shop_id', $shop_id);
        ...
        if ($is_quick == 1) {
            $this->db->where('c.is_quick_link', 1);
        } elseif ($is_quick == 0) {
            $this->db->where('c.is_quick_link', 0);
        }
        $initialQuery = $this->db->get();
        $menuList = $initialQuery->result_array();

        $resultArray = array();

        foreach ($menuList as $value) {
            if ($value['is_dropdown'] == 0 && $value['dropdown_id'] == 0) {
                $resultArray[] = $value;
            } elseif ($value['is_dropdown'] == 1 && $value['dropdown_id'] == 0) {
                $this->db->select('c.*');
                $this->db->from('vendor_menu_list c');
                $this->db->where('c.language', $this->site_lang);
                $this->db->where('c.dropdown_id', $value['id']);
                $dropdownQuery = $this->db->get();
                $dropdownList = $dropdownQuery->result_array();
                $value['dropdownList'] = $dropdownList;
                $resultArray[] = $value;
            }
        }
    ...
    public function get_order_type_by_type_id($type_id, $shop_id)
    {
        $this->db->select('u.*,u.id as user_type_id,ot.name as type_name,ot.slug,ot.id as order_type_id');
        $this->db->from('users_active_order_types u');
        $this->db->join('order_types ot', 'ot.id = u.type_id', 'LEFT');
    ...
    public function get_my_addons($item_id, $shop_id, $is_status = true)
    {
        $this->db->select('e.*');
        $this->db->from('extra_title_list e');
        $this->db->where('e.item_id', $item_id);
        $this->db->where('e.shop_id', $shop_id);
        $this->db->order_by('e.orders', 'ASC');
        $query = $this->db->get();
        $query = $query->result();
        foreach ($query as $key => $value) :
            $this->db->select('ex.*');
            $this->db->from('item_extras ex');
            $this->db->where('ex.item_id', $value->item_id);
            $this->db->where('ex.shop_id', $value->shop_id);
            $this->db->where('ex.extra_title_id', $value->id);
            if ($is_status == true):
                $this->db->where('ex.status', 1);
            endif;
            $this->db->order_by('ex.id', 'ASC');
            $query2 = $this->db->get();
            $query2 = $query2->result();
            $query[$key]->extras = $query2;
        endforeach;
        return $query;
    }

    public function check_existing_addons($item_id, $shop_id)
    {
        $this->db->select('u.*');
        $this->db->from('item_extras u');
        $this->db->where('u.item_id', $item_id);
        $this->db->where('u.shop_id', $shop_id);
        $this->db->where('u.extra_title_id', 0);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $query = $query->result();
            foreach ($query as $key => $value) :
                $this->db->delete('item_extras', array('id' => $value->id));
            endforeach;
            return true;
        } else {
            return 1;
    ...
    public function get_extra_title($item_id, $shop_id)
    {
        $this->db->select('u.*');
        $this->db->from('extra_title_list u');
        $this->db->where('u.item_id', $item_id);
        $this->db->where('u.shop_id', $shop_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
    ...
    public function check_empty_cities()
    {
        $users = $this->db->where('user_role', 0)->get('users');
        if ($users->num_rows() > 0) {
            $cities = $this->db->get('restaurant_city_list');
    ...
    {
        $users = $this->db->where(['is_default' => 1, 'shop_id' => restaurant()->id])->get('items');
        if ($users->num_rows() > 0) {
            $cities = $this->db->where(['is_default' => 1, 'user_id' => restaurant()->user_id])->get('menu_type');
            if ($cities->num_rows() > 0) {
                return 1;
            } else {
                return 0;
    ...
    public function insert_batch_data($data, $table)
    {
        $this->db->insert_batch($table, $data);

        $insertedIds = array();
        $startId = $this->db->insert_id();
        for ($i = $startId; $i < $startId + count($data); $i++) {
            $insertedIds[] = $i;
        }
    ...
    public function __ordertype($shop_id, $type_id, $type)
    {
        $info = $this->db->where(['shop_id' => $shop_id, 'type_id' => $type_id])->get('users_active_order_types');
        if ($info->num_rows() > 0) {
            $i = $info->row();
            return isset($i->$type) ? $i->$type : 0;
        } else {
            return 0;
        }
    }
    ...
    {
        $info = $this->db->where(['is_default' => 1])->get('packages');
        if ($info->num_rows() > 0) {
    ...
    // get message sender list for admin

    public function get_users_list()
    {

        $q = $this->input->get('search', true);
        $search = isset($q) && !empty($q) ? $q : '';
        $this->db->select('c.*,u.username,u.id as auth_id,u.name,u.thumb');
        $this->db->from('chat_list c');
        $this->db->join('users u', 'u.id = c.sender_id', 'LEFT');
        $this->db->where('c.role !=', 'admin');
        if (!empty($search)) :
            $this->db->like('TRIM(u.username)', trim($search))
                ->or_like('TRIM(u.name)', trim($search), 'before');
        endif;
        $this->db->order_by('c.created_at', 'DESC');
    ...
    public function get_sender_id($id)
    {
        ...
        $this->db->from('chat_list c');
        $this->db->join('users u', 'u.id = c.sender_id', 'LEFT');
        $this->db->where('md5(c.sender_id)', $id);
        $query = $this->db->get();
        $query = $query->row();
        if (!empty($id)) :
            return $query ?? '';
    ...
    public function get_user_message($sender_id = '')
    {
        $this->db->select('c.*,u.username,u.id as auth_id,u.name,u.thumb');
        $this->db->from('chat_list c');
        $this->db->join('users u', 'u.id = c.sender_id', 'LEFT');
        ...
        if (!empty($sender_id)) {
            $this->db->where('(c.sender_id = ' . $sender_id . ' OR c.receiver_id = ' . $sender_id . ')');
        }
    ...
    public function get_admin_user_message($sender_id = '', $receiver_id = '')
    {
        $this->db->select('c.*,u.username,u.id as auth_id,u.name,u.thumb');
        $this->db->from('chat_list c');
        $this->db->join('users u', 'u.id = c.sender_id', 'LEFT');
        ...
        if (!empty($sender_id) && !empty($receiver_id)) {
            $this->db->where("(c.sender_id = $sender_id AND c.receiver_id = $receiver_id) OR (c.sender_id = $receiver_id AND c.receiver_id = $sender_id)");
        }
    ...
    {
        $unseen_count = $this->db
            ->where(['sender_id' => $id, 'status' => 'unseen'])
            ->count_all_results('chat_list');

        $last_message = $this->db
            ->where(['sender_id' => $id, 'status' => 'unseen'])
        ...
        return ['last_message' => $last_message->message ?? '', 'count' => $unseen_count ?? 0];
    }


    // sub category

    public function get_my_sub_categories()
    {
        $shopId = restaurant()->id;
        $userId = auth('id');
        $lang = $this->site_lang;

        $this->db->select('s.*, s.id as sub_cat_id, m.name as category_name');
        $this->db->from('sub_category_list s');
        $this->db->join('menu_type m', 'm.category_id = s.category_id AND m.language = s.language', 'right');
        $this->db->where([
            's.user_id' => $userId,
            's.shop_id' => $shopId,
            's.language' => $lang,
    ...
    public function get_subcategories_by_cat_id($id, $shop_id, $is_md5 = false, $language = null)
    {
        $language = !empty($language) ? $language : $this->site_lang;
        $this->db->select('s.*');
        $this->db->from('sub_category_list as s');
        $this->db->where('s.shop_id', $shop_id);
        if ($is_md5 == true) {
            $this->db->where('md5(s.category_id)', $id);
        } else {

            $this->db->where('s.category_id', $id);
        }
    ...
    public function get_extras_by_title_id($id, $shop_id)
    {
        $this->db->select('i.*');
        $this->db->from('item_extras i');
        $this->db->where('extra_title_id', $id);
        $this->db->where('shop_id', $shop_id);
        $query = $this->db->get();
        $query = $query->result_array();
        return $query;
    ...
    {
        $this->db->select('i.latitude,longitude,id');
        $this->db->from('restaurant_list i');
        $this->db->where('i.latitude is NOT NULL', NULL, FALSE);
        $query = $this->db->get();
        $query = $query->result();
        return $query;
    }

    public function get_mailed_customers()
    {
        $shop_id = restaurant()->id;
    ...
    public function get_items_for_offers()
    {
        $shop_id = restaurant()->id;
        $this->db->select('*');
        $this->db->from('items');
    ...
    public function get_offers($shop_id)
    {
        $this->db->select('*');
        ...
        foreach ($query as $key => $row) {
            $this->db->select('i.title,i.id');
            $this->db->from('items i');
            $this->db->where_in('i.id', __jsonDecode($row['item_ids']));
            $query2 = $this->db->get();
            $query2 = $query2->result_array();
            $query[$key]['items'] = $query2;
        }
