...
class Pos_m extends CI_Model
{
	...
	public function get_my_all_items($catId = '', $limit = '', $per_page = 0, $offset = 0, $total = 0)
	{
		...
		if (isset($_GET['q']) && !empty($_GET['q'])) :
			if (is_numeric($_GET['q'])) {
				...
				$this->db->like('TRIM(i.title)', trim($_GET['q']));
			}
	...
	public function get_my_all_packages($limit = '', $per_page = 0, $offset = 0, $total = 0)
	{
		...
		if (isset($_GET['q']) && !empty($_GET['q'])) :
			if (is_numeric($_GET['q'])) {
				$this->db->where('ip.uid', $_GET['q']);
			} else {
				$this->db->like('TRIM(ip.package_name)', trim($_GET['q']));
			}
		...
		$this->db->where('ip.shop_id', $shop_id);

		$this->db->where('ip.language', $this->site_lang);
	...
	public function get_item_name()
	{
		...
		if (isset($_GET['itemName']) && !empty(isset($_GET['itemName']))) {
			$this->db->like('TRIM(s.title)', trim($_GET['itemName']));
		}
	...
	public function order_qr($orderId, $shopId)
	{
		...
		if (isset($shop['is_order_qr']) && $shop['is_order_qr'] == 1):
			...
			$qr_image = $shop['username'] . '_' . random_string('numeric', 8) . '.png';
	...
	public function get_my_expenses()
	{
		...
		$this->db->where('ex.shop_id', $_ENV['ID']);
