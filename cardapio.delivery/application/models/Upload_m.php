...
class Upload_m extends CI_Model
{
	...
	public function order_qr($phone, $orderId, $shopId)
	{
		...
		if (isset($shop['is_order_qr']) && $shop['is_order_qr'] == 1) {
			$this->load->library('ciqrcode');
			$qr_image = $shop['username'] . '_' . random_string() . '.png';
			$params['data'] = asset_url('my-orders/' . $shop['username'] . '?phone=' . $phone . '&orderId=' . $orderId);
			$params['level'] = 'H';
			$params['size'] = 8;
	...
	public function menu_qr($menuId, $shopId)
	{
		$shop = $this->common_m->get_restaurant_info_by_id($shopId);
		$this->load->library('ciqrcode');
		$qr_image = $shop['username'] . '_menu_' . random_string() . '.png';
		$params['data'] = asset_url('qr-menu/' . $shop['username'] . '/' . md5($menuId));
		$params['level'] = 'H';
		$params['size'] = 12;
		$params['savename'] = FCPATH . "uploads/orders_qr/" . $qr_image;
		if ($this->ciqrcode->generate($params)) {
	...
	public function croped__()
	{
		$this->load->library('image_lib');
		// Retrieve other form data if needed

		$croppedImageData = $this->input->post('cropData');

		// Resize the image
		$resizeConfig['width'] = 150;  // Adjust the width as needed
		$resizeConfig['height'] = 150;  // Adjust the height as needed
		$resizeConfig['quality'] = '90%';
		$resizeConfig['source_image'] = $croppedImageData;  // Use the base64-encoded image data directly
		...
		$uploadConfig['file_name'] = 'resized_image.jpg';  // Adjust the filename as needed

		$this->load->library('upload', $uploadConfig);

		if (!$this->upload->do_upload('cropData')) {
