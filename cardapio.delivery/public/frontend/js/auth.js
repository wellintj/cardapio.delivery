(function ($) {
  ...
  **/
  ...
  // Inicialização do datepicker quando o documento estiver pronto
  $(document).ready(function() {
    // Inicializar datepicker se existir na página
  ...
  function init_datepicker() {
    ...
    if (typeof flatpickr === 'undefined') {
      console.warn('Flatpickr não está carregado. Tentando carregar dinamicamente...');
      
      // Carregar CSS do Flatpickr
      if (!$('link[href*="flatpickr.min.css"]').length) {
        $('head').append('<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">');
      }
      
      // Carregar JS do Flatpickr
      $.getScript('https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.js')
        .done(function() {
          // Carregar localização pt-BR
          $.getScript('https://cdn.jsdelivr.net/npm/flatpickr/dist/l10n/pt-BR.js')
            .done(function() {
              console.log('Flatpickr e localização pt-BR carregados com sucesso');
              init_datepicker_config();
            })
            .fail(function() {
              console.error('Erro ao carregar a localização pt-BR');
              init_datepicker_config();
            });
        })
        .fail(function() {
          console.error('Erro ao carregar o Flatpickr');
        });
  ...
  function init_datepicker_config() {
    var day = $('.off_days').data('day') || [];
    
    // Definir a localização brasileira manualmente
    var ptBR = {
      weekdays: {
    ...
    // Aplicar a localização manualmente
    if (typeof flatpickr !== 'undefined') {
      flatpickr.localize(ptBR);
    }
    
    // Configurações padrão para o datepicker
    var config = {
      enableTime: false,
      dateFormat: "Y-m-d",
      altInput: true,
    ...
    try {
      // Inicializar o Flatpickr com as configurações
      $(".datepicker-1").flatpickr(config);
    } catch (error) {
  ...
  function show_date_details() {
    ...
    $('form').off('submit.datepicker').on('submit.datepicker', function () {
      $(".datepicker-1").each(function () {
        const pickerInstance = $(this).data("flatpickr");
        if (pickerInstance) {
    ...
    // Definir localização brasileira para o timepicker
    var ptBR = {
      weekdays: {
        shorthand: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
        longhand: ['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado']
    ...
    try {
      ...
      console.error('Erro ao inicializar o timepicker:', error);
    }
  }
  ...
  $(document).ready(function () {
    // Verifique se venobox está disponível antes de usá-lo
  ...
  $(function () {
    ...
    // Definir localização brasileira
    var ptBR = {
      weekdays: {
        shorthand: ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'],
        longhand: ['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado']
    ...
    // Inicializar datepicker para a página de reservas (usa o ID datetimepicker1)
    if($("#datetimepicker1").length > 0) {
      $("#datetimepicker1 .datetimepicker").flatpickr({
        enableTime: true,
        dateFormat: "Y-m-d H:i",
        altInput: true,
        altFormat: "d/m/Y H:i",
        ...
        "onChange": [function (selectedDates, dateStr, instance) {
          // Garantir que o formato da data no campo original seja Y-m-d H:i
  ...
  $(document).on('submit', '.sendOtp', function (e) {
    ...
    if ($(this).find('.reg_msg').length == 0) {
      console.log('reg_msg');
      $(this).append('<span class="reg_msg"></span>');
    }
    var url = $(this).attr('action');
    $.post(url, $(this).serialize(), function (json) {
      if (json.st == 1) {
        ...
        $('.showOtpContent').html(json.load_data);
  ...
  $(document).on('submit', '.vefiryOtp', function (e) {
    e.preventDefault();
    $(this).find('button[type="submit"]').prop('disabled', true);
    if ($(this).find('.reg_msg').length == 0) {
      $(this).append('<span class="reg_msg"></span>');
    ...
    $.post(url, $(this).serialize(), function (json) {
      if (json.st == 1) {
        if (json.isLoad == 1) {
          $('.showOtpContent').html(json.load_view);
        } else {
  ...
  $(document).on('submit', '.add_otp_info', function (e) {
    e.preventDefault();
    if ($(this).find('.reg_msg').length == 0) {
      $(this).append('<span class="reg_msg"></span>');
    }
