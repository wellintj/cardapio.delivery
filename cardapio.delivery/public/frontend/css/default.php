...
if (isset($_GET['color'])) {
	$color ='#' . $_GET['color'] ?? '50cd89';
	$hex_color =$_GET['color'] ?? '50cd89';
}else {
	$color ='#50cd89';
	$hex_color ='50cd89';
}
...
?><?php function hex2rgb($color) {
	// Ensure $color is a valid string and remove "#" if present
	$color =ltrim($color, '#');

	// Validate that $color is exactly 6 characters long and contains only hex digits
	if (strlen($color) !==6 || !ctype_xdigit($color)) {
		return '0, 0, 0'; // Return black for invalid input
	}

	// Extract and convert hex values to decimal
	$r =hexdec(substr($color, 0, 2)); // First 2 characters
	$g =hexdec(substr($color, 2, 2)); // Middle 2 characters
	$b =hexdec(substr($color, 4, 2)); // Last 2 characters
...
.cartNofify.notifyBadge {
	background: rgba(<?=hex2rgb($hex_color); ?>, .9) !important;
}

.cartQty {
	border-radius: 100%;
	background: radial-gradient(rgba(<?=hex2rgb($hex_color); ?>, 1) 55%, rgba(255, 255, 255, .5) 58%);
	min-width: 33px;
	text-align: center;
}


li.cart.navCart.CartIcon.active.menu_style_3 {
	background: rgba(<?=hex2rgb($hex_color); ?>, 1) !important;
	color: #fff !important;
	box-shadow: 0 4px 5px rgba(<?=hex2rgb($hex_color); ?>, .8), 0 2px 6px rgb(255 255 255 / 8%);
}

li.cart.navCart.CartIcon.active.menu_style_3 i {
	color: #fff !important;
}

li.cart.navCart.CartIcon.active.menu_style_3 span.total_count {
...
?><?php if ($theme_color ==1) : ?>section.services_area.how_it_works {
...
<?php if (isset($themes) && $themes ==1) : ?>.theme_1 .itemPrice_area.i_small {
	flex-wrap: nowrap;
	white-space: nowrap;
	overflow-x: auto;
}
...
<?php endif ?><?php if (isset($themes) && $themes ==2) : ?>.homeItemDetails.list_view .itemPrice_area.i_small {
	flex-wrap: nowrap;
	white-space: nowrap;
	overflow-x: auto;
}

<?php endif ?><?php if (isset($themes) && $themes ==3) : ?><?php endif ?>button.btn.btn-primary.add_to_cart_btn:hover {
	background: #fff !important;
	color: rgba(<?=hex2rgb($hex_color); ?>, 1) !important;
