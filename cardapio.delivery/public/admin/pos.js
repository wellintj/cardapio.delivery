(function ($) {
	...
	// Configuração global do Flatpickr para o idioma português
	if (typeof flatpickr !== 'undefined') {
		// Definição do locale em português
		const Portuguese = {
			weekdays: {
		...
		console.log('Flatpickr Português inicializado globalmente');
	}
	...
	var beep = `${base_url}assets/frontend/mp3/beep.mp3`;
	...
	$(function () {
		$(document).on('keyup', '#search-box', function () {
			...
			$.ajax({
				type: "GET",
				dataType: 'json',
				url: `${base_url}admin/pos/get_item_name`,
				data: formData, processData: false,  // Important! Set processData to false
	...
	$(document).on('click', '.getItems', function () {
		...
		var url = `${base_url}admin/pos/ajax_pagination?type=${type}`;
		$.get(url, {
			'csrf_test_name': csrf_value
		}, function (json) {
			if (json.st == 1) {
				$('.menuItemList ul li a').removeClass('active');
				$('.posCatList li a').removeClass('active');
	...
	$(document).on('click', '.add_to_cart', function () {
		...
		var url = `${base_url}admin/pos/add_to_cart/${id}/${type}`;
	...
	$(document).on('input', '.receivedAmount', function () {
		...
		if (isEmpty(val) == false) {
			...
			$('.changeReturn').text(getPrice(amount));
		} else {
			$('.changeReturn').text(0);
		}
	});
	...
	$(document).on('submit', '.addOrderForm', function () {
		...
		$.post(url, $(this).serialize() + '&is_draft=' + submitButtonValue, function (json) {
			if (json.st == 1) {
	...
	function showPrice(price) {
		$('.show_price').text(getPrice(price));
		$('.item_price').val(getPrice(price));
	}
	...
	function pickupTime() {
		...
		// Certifique-se que o flatpickr tenha sido inicializado
		if (typeof flatpickr === 'undefined') {
			console.error('Flatpickr não está carregado');
			return;
		}
		
		// Configuração específica para o locale PT
		const Portuguese = {
			weekdays: {
				shorthand: ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"],
				longhand: [
		...
		// Configurações comuns para ambos os datepickers
		const dateConfig = {
			enableTime: false,
			dateFormat: "Y-m-d",
			altInput: true,
		...
		const picker3Elements = document.querySelectorAll('.datepicker-3');
		picker3Elements.forEach(element => {
			if (element._flatpickr) {
				element._flatpickr.destroy();
			}
		});
		
		const picker1Elements = document.querySelectorAll('.datepicker-1');
		picker1Elements.forEach(element => {
			if (element._flatpickr) {
				element._flatpickr.destroy();
			}
		});
		
		// Inicializar datepicker-3
		$(".datepicker-3").flatpickr(dateConfig);
		
		// Inicializar datepicker-1
		$(".datepicker-1").flatpickr(dateConfig);
		...
		$.get(url, { 'csrf_test_name': csrf_value }, function (json) {
			if (json.st == 1) {
				setTimeout(() => {
					...
					$('.itemListArea').removeClass('null');
					$('.itemListArea').html(json.result);
					var $menuItem = $('.menuItemList ul li a[data-type="item"]');
					if ($menuItem.length > 0) {
						$menuItem.addClass('active');
					} else {
						console.error("No element found matching $('.menuItemList ul li a[data-type=\"items\"]')");
					}
				}, 1000);
			} else {
				console.error("JSON response indicates failure: ", json);
			}
		}, 'json').fail(function (jqXHR, textStatus, errorThrown) {
	...
	function beepRing() {
		try {
			...
			audio.play();
			console.log("added"); // Debug statement
		} catch (error) {
			console.error("Error playing sound:", error); // Log any errors to console
		}
	}
